...
"""Command to get a SHA effective custom module."""
...
class DescribeEffective(base.DescribeCommand):
  ...
  def Args(parser):
    flags.CreateModuleIdOrNameArg(
        module_type=constants.CustomModuleType.EFFECTIVE_SHA
    ).AddToParser(parser)
    flags.CreateParentFlag(required=False).AddToParser(parser)

  def Run(self, args):
    name = parsing.GetModuleNameFromArgs(
        args, module_type=constants.CustomModuleType.EFFECTIVE_SHA
    )

    client = clients.EffectiveSHACustomModuleClient()
