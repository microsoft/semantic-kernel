...
class AppEngineHttpRequest(_messages.Message):
  ...
  the task will be sent to is: * `url =` host `+` relative_uri Tasks can be
...
class HttpRequest(_messages.Message):
  ...
  Messages:
    HeadersValue: HTTP request headers. This map contains the header field
      ...
      `"application/json"`. Headers which can have multiple values (according
      to RFC2616) can be specified using comma-separated values. The size of
  ...
  class HeadersValue(_messages.Message):
    ...
    Google use only. * `X-AppEngine-*`: Google use only. `Content-Type` won't
    be set by Cloud Tasks. You can explicitly set `Content-Type` to a media
    type when the task is created. For example, `Content-Type` can be set to
...
class RateLimits(_messages.Message):
  ...
  Fields:
    ...
    maxDispatchesPerSecond: The maximum rate at which tasks are dispatched
      ...
      value is 500. This field has the same meaning as [rate in queue.yaml/xml
      ](https://cloud.google.com/appengine/docs/standard/python/config/queuere
      f#rate).
  """
...
class Task(_messages.Message):
  ...
  Fields:
    ...
    pullMessage: Pull Message contained in a task in a PULL queue type. This
      ...
      tasks through the CloudTasks.GetTask.
    ...
    scheduleTime: The time when the task is scheduled to be attempted. For App
      Engine queues, this is when the task will be attempted or retried.
      `schedule_time` will be truncated to the nearest microsecond.
