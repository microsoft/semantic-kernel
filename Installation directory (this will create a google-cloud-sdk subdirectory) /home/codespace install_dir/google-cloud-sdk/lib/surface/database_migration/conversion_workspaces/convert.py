...
class Convert(base.Command):
  ...
  def Run(self, args):
    ...
    cw_client = conversion_workspaces.ConversionWorkspacesClient(
        self.ReleaseTrack())
    result_operation = cw_client.Convert(
    ...
    if args.IsKnownAndSpecified('no_async'):
      log.status.Print(
          'Waiting for conversion workspace [{}] to be converted with [{}]'
          .format(
      ...
      log.status.Print(
          'Converted conversion workspace {} [{}]'.format(
              conversion_workspace_ref.conversionWorkspacesId,
              result_operation.name,
          )
      )
