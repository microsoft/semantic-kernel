...
def clone(
    ...
    **kwargs
    ...
    """
    # TODO(user): This code overlaps quite a bit with Repo.clone
...
# TODO(user): better default for encoding?
def show(
    repo=".",
    objects=None,
    outstream=sys.stdout,
...
def update_head(repo, target, detached=False, new_branch=None):
    ...
    with open_repo_closing(repo) as r:
        if new_branch is not None:
            ...
            to_set = b"HEAD"
        if detached:
            # TODO(user): Provide some way so that the actual ref gets
...
def fsck(repo):
    ...
    """
    with open_repo_closing(repo) as r:
        # TODO(user): check pack files
        # TODO(user): check graph
        # TODO(user): check refs
