...
SIGNATURE_METHODS = (SIGNATURE_HMAC_SHA1, SIGNATURE_HMAC_SHA256, SIGNATURE_RSA,
                     SIGNATURE_PLAINTEXT)
...
class Client(object):
  ...
  def get_oauth_params(self, request):
    ...
    timestamp = (
        generate_timestamp() if self.timestamp is None else self.timestamp)
    params = [
        ('oauth_nonce', nonce),
    ...
    if request.body is not None and content_type_eligible:
      params.append(
          ('oauth_body_hash',
           base64.b64encode(
               hashlib.sha1(
                   request.body.encode('utf-8')).digest()).decode('utf-8')))
  ...
  def _render(self, request, formencode=False, realm=None):
    ...
    if self.signature_type == SIGNATURE_TYPE_AUTH_HEADER:
      ...
      body = parameters.prepare_form_encoded_body(request.oauth_params,
