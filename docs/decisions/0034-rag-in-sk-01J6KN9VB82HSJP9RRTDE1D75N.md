...
runme:
  document:
    relativePath: 0034-rag-in-sk.md
  session:
    id: 01J6KN9VB82HSJP9RRTDE1D75N
    updated: 2024-08-31 07:42:08Z
status: proposed
---
...
There are 12 [vector DB connectors](ht************************************************************************rs) (also known as `memory connectors`) implemented at the moment, and it may be unclear for developers how to use them. It's possible to call connector methods directly or use it via `TextMemoryPlugin` from [Plugins.Memory](ht******************************************************************ry) NuGet package (prompt example: `{{recall 'company budget by year'}} What is my budget for 2024?`)
...
By implementing the same interface, each integration is aligned, which makes it possible to use different vector DBs at runtime. At the same time it is disadvantage, because each vector DB can work differently, and it becomes harder to fit all integrations into already existing abstraction. For example, method `CreateCollectionAsync` from `IMemoryStore` is used when application tries to add new record to vector DB to the collection, which doesn't exist, so before insert operation, it creates new collection. In case of [Pinecone](ht*******************io/) vector DB, this scenario is not supported, because Pinecone index creation is an asynchronous process - API service will return 201 Created HTTP response with following property in response body (index is not ready for usage):
...
- [ChatGPT Retrieval Plugin](ht**********************************************in) - this plugin should be hosted as a separate service. It has integration with various [vector databases](ht********************************************************************************************se).
- [SemanticKernel.Plugins.Memory.TextMemoryPlugin](ht******************************************************************ry) - Semantic Kernel solution, which supports various [vector databases](ht*********************************************************************************************************es).
- Custom user plugin.
...
var memory = new MemoryBuilder()
    .WithChromaMemoryStore("ht*******************nt")
    .WithOpenAITextEmbeddingGeneration("te******************02", "api-key")
    .Build();
...
var executionSettings = new OpenAIPromptExecutionSettings
{
    ...
    MemoryConfig = new()
    {
        ...
        Memory = new ChromaMemoryStore("ht*******************nt"),
