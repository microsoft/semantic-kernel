...
public class VectorSearch_Simple(ITestOutputHelper output, VectorStoreQdrantContainerFixture qdrantFixture) : BaseTest(output), IClassFixture<VectorStoreQdrantContainerFixture>
{
    ...
    public async Task ExampleAsync()
    {
        ...
        var vectorSearch = collection as IVectorSearch<Glossary>;

        // Search the collection using a vector search.
        var searchString = "What is an Application Programming Interface";
        var searchVector = await textEmbeddingGenerationService.GenerateEmbeddingAsync(searchString);
        var searchResult = await vectorSearch!.SearchAsync(searchVector, new() { Limit = 1 }).ToListAsync();
        ...
        searchResult = await vectorSearch!.SearchAsync(searchVector, new() { Limit = 1 }).ToListAsync();
