...
public class ProcessController : ControllerBase
{
    ...
    public async Task<IActionResult> PostAsync(string processId)
    {
        ...
        var processContext = await process.StartAsync(new KernelProcessEvent() { Id = CommonEvents.StartProcess }, processId);
    ...
    /// <param name="processId">The Id of the process.</param>
    /// <returns></returns>
    [HttpGet("errorf/{processId}")]
    public async Task<IActionResult> FunctionErrorAsync(string processId)
    {
        this._kernel.Data.Clear();
        var process = this.GetFunctionErrorProcess("Start");
        var processContext = await process.StartAsync(new KernelProcessEvent() { Id = "Start", Data = processId }, processId);
        var finalState = await processContext.GetStateAsync();
        var errorHandled = this._kernel.Data.ContainsKey("error-function");

        Console.WriteLine("##### CONTROLLER COMPLETE");

        return this.Ok(processId + " " + errorHandled);
    }
    ...
    /// <param name="processId">The Id of the process.</param>
    /// <returns></returns>
    [HttpGet("errorp/{processId}")]
    public async Task<IActionResult> ProcessErrorAsync(string processId)
    {
        this._kernel.Data.Clear();
        var process = this.GetGlobalErrorProcess("Start");
        var processContext = await process.StartAsync(new KernelProcessEvent() { Id = "Start", Data = processId }, processId);
        var finalState = await processContext.GetStateAsync();
        var errorHandled = this._kernel.Data.ContainsKey("error-global");
    ...
    /// <param name="processId">The Id of the process.</param>
    /// <returns></returns>
    [HttpGet("errorb/{processId}")]
    public async Task<IActionResult> PrecedenceErrorAsync(string processId)
    {
        this._kernel.Data.Clear();
        var process = this.GetBothErrorProcess("Start");
        var processContext = await process.StartAsync(new KernelProcessEvent() { Id = "Start", Data = processId }, processId);
        var finalState = await processContext.GetStateAsync();
        var errorHandled = this._kernel.Data.ContainsKey("error-function") && !this._kernel.Data.ContainsKey("error-global");
    ...
    private KernelProcess GetFunctionErrorProcess(string initialEvent)
    {
        ProcessBuilder process = new("ErrorTest");

        ProcessStepBuilder testStep = process.AddStepFromType<FailedStep>();
        process.OnInputEvent(initialEvent).SendEventTo(new ProcessFunctionTargetBuilder(testStep));
    ...
    private KernelProcess GetGlobalErrorProcess(string initialEvent)
    {
        ProcessBuilder process = new("ErrorTest");
    ...
    private KernelProcess GetBothErrorProcess(string initialEvent)
    {
        ProcessBuilder process = new("ErrorTest");
    ...
    private sealed class FailedStep : KernelProcessStep
    {
        ...
        //public void TestFailure(Exception exception)
        {
            //Console.WriteLine("##### STEP ERROR RAISED!!!" + exception?.GetType().Name ?? "*");
            Console.WriteLine("##### TEST ERROR RAISED!!! " + value);
            throw new InvalidOperationException("I failed!: " + value);
        }
    }
    ...
    private sealed class ErrorStep : KernelProcessStep
    {
        ...
        {
            Console.WriteLine("##### GLOBAL ERROR HANDLER.");
        ...
        {
            Console.WriteLine("##### FUNCTION ERROR HANDLER.");
