...
{
    public static class FormFillingProcessEvents
    {
        public const string OnStartProcess = nameof(OnStartProcess);
    }

    private static KernelProcess GetExperimentalProxyProcess()
    {
        ProcessBuilder processBuilder = new("proxyProcess");
        var fillFormStep = processBuilder.AddStepFromType<CompleteNewCustomerFormStep>();
        var proxyStep = processBuilder.AddProxyStepFromType<AlternativeProxyStep, FormCompletionEvents>("https://localhost:58844/chatHub");

        processBuilder
            .OnInputEvent(FormFillingProcessEvents.OnStartProcess)
            .SendEventTo(new ProcessFunctionTargetBuilder(fillFormStep, functionName: CompleteNewCustomerFormStep.Functions.NewAccountWelcome));

        processBuilder
            .OnInputEvent(FormFillingProcessEvents.OnStartProcess)
            .InitializeProxy(proxyStep);

        fillFormStep
            .OnEvent(CompleteNewCustomerFormStep.OutputEvents.NewCustomerFormWelcomeMessageComplete)
            .EmitToProxy(proxyStep, FormCompletionEvents.OnEmitAssistantMessageWithUserReply);

        fillFormStep
            .OnEvent(CompleteNewCustomerFormStep.OutputEvents.NewCustomerFormNeedsMoreDetails)
            .EmitToProxy(proxyStep, FormCompletionEvents.OnEmitAssistantMessageWithUserReply);

        fillFormStep
            .OnEvent(CompleteNewCustomerFormStep.OutputEvents.NewCustomerFormCompletedAssistantMessage)
            .EmitToProxy(proxyStep, FormCompletionEvents.OnEmitAssistantMessageWithoutUserReply);

        fillFormStep
            .OnEvent(CompleteNewCustomerFormStep.OutputEvents.NewCustomerFormCompleted)
            .EmitToProxy(proxyStep, FormCompletionEvents.OnFormCompleted);

        proxyStep
            .OnEvent(FormCompletionEvents.OnUserResponseReceived)
            .SendEventTo(new ProcessFunctionTargetBuilder(fillFormStep, functionName: CompleteNewCustomerFormStep.Functions.NewAccountProcessUserInfo));

        return processBuilder.Build();
    }

    private static KernelProcess GetFormFillingProcess()
    {
        ProcessBuilder processBuilder = new("FormFillingWithDapr");

        var fillFormStep = processBuilder.AddStepFromType<CompleteNewCustomerFormStep>();
        var proxyStep = processBuilder.AddStepFromType<HackyProxyStep>();
        ...
        fillFormStep
            .OnEvent(CompleteNewCustomerFormStep.OutputEvents.NewCustomerFormWelcomeMessageComplete)
            .SendEventTo(new ProcessFunctionTargetBuilder(proxyStep, functionName: HackyProxyStep.Functions.EmitWelcomeMessage));

        proxyStep
            .OnFunctionResult(HackyProxyStep.Functions.EmitWelcomeMessage)
            .SendEventTo(new ProcessFunctionTargetBuilder(fillFormStep, functionName: CompleteNewCustomerFormStep.Functions.NewAccountProcessUserInfo));

        fillFormStep
            .OnEvent(CompleteNewCustomerFormStep.OutputEvents.NewCustomerFormNeedsMoreDetails)
            .SendEventTo(new ProcessFunctionTargetBuilder(proxyStep, functionName: HackyProxyStep.Functions.EmitNewFormAssistantMessage));

        proxyStep
             .OnFunctionResult(HackyProxyStep.Functions.EmitNewFormAssistantMessage)
             .SendEventTo(new ProcessFunctionTargetBuilder(fillFormStep, functionName: CompleteNewCustomerFormStep.Functions.NewAccountProcessUserInfo));

        fillFormStep
            .OnEvent(CompleteNewCustomerFormStep.OutputEvents.NewCustomerFormCompletedAssistantMessage)
            .SendEventTo(new ProcessFunctionTargetBuilder(proxyStep, functionName: HackyProxyStep.Functions.EmitNewFormAssistantMessageNoReply));

        fillFormStep
            .OnEvent(CompleteNewCustomerFormStep.OutputEvents.NewCustomerFormCompleted)
            .SendEventTo(new ProcessFunctionTargetBuilder(proxyStep, functionName: HackyProxyStep.Functions.EmitCompletedForm));

        var kernelProcess = processBuilder.Build();
        return kernelProcess;
    }

    private static KernelProcess GetProcess()
    {
    ...
    public static class CommonEvents
    {
        ...
        public const string CStepExternalInput = nameof(CStepExternalInput);
    ...
    public static async Task<DaprKernelProcessContext> StartProcessAsync(string processId)
    {
    ...
    public static async Task<DaprKernelProcessContext> StartFormFillingProcessAsync(string processId)
    {
        // estenori-TODO: switch to experimental process when fully hooked up
        //var process = GetExperimentalProxyProcess() -> internals not fully hooked up yet
        var process = GetFormFillingProcess();
        var processContext = await process.StartAsync(new KernelProcessEvent() { Id = FormFillingProcessEvents.OnStartProcess }, processId: processId);
        return processContext;
    }
}
