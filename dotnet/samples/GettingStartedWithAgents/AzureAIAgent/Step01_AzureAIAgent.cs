...
public class Step01_AzureAIAgent(ITestOutputHelper output) : BaseAzureAgentTest(output)
{
    ...
    public async Task UseTemplateForAzureAgentAsync()
    {
        ...
        AzureAIAgent agent = new(definition, this.AgentsClient, templateFactory: new KernelPromptTemplateFactory(), templateFormat: PromptTemplateConfig.SemanticKernelTemplateFormat)
        {
            Arguments =
            {
                { "topic", "Dog" },
