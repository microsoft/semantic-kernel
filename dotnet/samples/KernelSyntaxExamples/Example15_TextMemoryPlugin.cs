...
public static class Example15_TextMemoryPlugin
{
    ...
    private static async Task RunWithStoreAsync(IMemoryStore memoryStore, CancellationToken cancellationToken)
    {
        ...
        var result = await kernel.InvokeAsync(memoryFunctions["Retrieve"], new KernelFunctionArguments()
        {
            [TextMemoryPlugin.CollectionParam] = MemoryCollectionName,
            [TextMemoryPlugin.KeyParam] = "info5"
        }, cancellationToken);
        ...
        result = await kernel.InvokeAsync(memoryFunctions["Recall"], new KernelFunctionArguments()
        {
            [TextMemoryPlugin.CollectionParam] = MemoryCollectionName,
