...
public static class Example21_ChatGptPlugins
{
    ...
    private static async Task RunChatGptPluginAsync()
    {
        ...
        var skill = await kernel.ImportAIPluginAsync("<skill name>", new Uri("<chatGPT-plugin>"), new OpenApiPluginExecutionParameters(httpClient) { EnableDynamicPayload = true });
        ...
        //var skill = await kernel.ImportAIPluginAsync("Klarna", new Uri("https://www.klarna.com/.well-known/ai-plugin.json"), new OpenApiPluginExecutionParameters(httpClient));
