...
public static class Example30_ChatWithPrompts
{
    public static async Task RunAsync()
    {
        ...
        string systemMessage = await promptTemplateFactory.Create(new PromptModel(systemPromptTemplate)).RenderAsync(kernel, variables);
        Console.WriteLine($"------------------------------------\n{systemMessage}");
        ...
        string userMessage = await promptTemplateFactory.Create(new PromptModel(userPromptTemplate)).RenderAsync(kernel, variables);
        Console.WriteLine($"------------------------------------\n{userMessage}");
