...
public static class Example49_LogitBias
{
    public static async Task RunAsync()
    {
        ...
        var chatHistory = chatCompletionService.CreateNewChat("You are a librarian expert");

        // First user message
        chatHistory.AddUserMessage("Hi, I'm looking some suggestions");
        await MessageOutputAsync(chatHistory);

        string reply = await chatCompletionService.GenerateMessageAsync(chatHistory, settings);
        ...
        reply = await chatCompletionService.GenerateMessageAsync(chatHistory, settings);
        chatHistory.AddAssistantMessage(reply);
        await MessageOutputAsync(chatHistory);
