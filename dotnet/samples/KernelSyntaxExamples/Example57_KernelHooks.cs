...
public static class Example57_KernelHooks
{
    ...
    private static async Task GetUsageAsync()
    {
        ...
        {
            Console.WriteLine($"{e.Function.Metadata.PluginName}.{e.Function.Metadata.Name} : Pre Execution Handler - Triggered");
        }

        void MyRemovedPreExecutionHandler(object? sender, FunctionInvokingEventArgs e)
        {
            Console.WriteLine($"{e.Function.Metadata.PluginName}.{e.Function.Metadata.Name} : Pre Execution Handler - Should not trigger");
            e.Cancel();
        }
        ...
        void MyPostExecutionHandler(object? sender, FunctionInvokedEventArgs e)
        {
            ...
            Console.WriteLine($"{e.Function.Metadata.PluginName}.{e.Function.Metadata.Name} : Post Execution Handler - Total Tokens: {modelResults?.First().GetOpenAIChatResult().Usage.TotalTokens}");
        }
    ...
    private static async Task GetRenderedPromptAsync()
    {
        ...
        {
            Console.WriteLine($"{e.Function.Metadata.PluginName}.{e.Function.Metadata.Name} : Prompt Rendering Handler - Triggered");
            e.Variables.Set("style", "Seinfeld");
        }

        void MyRenderedHandler(object? sender, PromptRenderedEventArgs e)
        {
            Console.WriteLine($"{e.Function.Metadata.PluginName}.{e.Function.Metadata.Name} : Prompt Rendered Handler - Triggered");
    ...
    private static async Task BeforeInvokeCancellationAsync()
    {
        ...
        {
            Console.WriteLine($"{e.Function.Metadata.PluginName}.{e.Function.Metadata.Name} : FunctionInvoking - Cancelling all subsequent invocations");
            e.Cancel();
        };
