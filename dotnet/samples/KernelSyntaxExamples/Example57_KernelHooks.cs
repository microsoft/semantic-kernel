...
public static class Example57_KernelHooks
{
    ...
    private static async Task GetUsageAsync()
    {
        ...
        void MyPostExecutionHandler(object? sender, FunctionInvokedEventArgs e)
        {
            ...
            Console.WriteLine($"{e.Function.Name} : Post Execution Handler - Total Tokens: {modelResults?.First().GetOpenAIChatResult().Usage.TotalTokens}");
        }
    ...
    private static async Task BeforeInvokeCancellationAsync()
    {
        ...
        {
            Console.WriteLine($"{e.Function.Name} : FunctionInvoking - Cancelling all subsequent invocations");
            e.Cancel();
        };
