...
";
        ...
        await NativeStreamingValueTypeAsync(kernel, functions, cancellationToken);

        await NativeStreamingComplexTypeAsync(kernel, functions, cancellationToken);

        await NativeValueTypeAsync(kernel, functions, cancellationToken);

        await NativeComplexTypeAsync(kernel, functions, cancellationToken);
    }
    ...
    private static async Task NativeValueTypeAsync(IKernel kernel, IDictionary<string, ISKFunction> functions, CancellationToken cancellationToken)
    {
        ...
        await foreach (var update in kernel.StreamingRunAsync(functions["MyValueTypeNativeFunction"], new ContextVariables("My Value Type Non Streaming Function Input"), cancellationToken))
        {
            // the complex type will be available thru the Value property of the native update abstraction.
            if (update.Type == "native_result_update" && update is StreamingNativeResultUpdate nativeUpdate)
            {
    ...
    private static async Task NativeComplexTypeAsync(IKernel kernel, IDictionary<string, ISKFunction> functions, CancellationToken cancellationToken)
    {
        ...
        await foreach (var update in kernel.StreamingRunAsync(functions["MyComplexTypeNativeFunction"], new ContextVariables("My Complex Type Non Streaming Function Input"), cancellationToken))
        {
            // the complex type will be available thru the Value property of the native update abstraction.
