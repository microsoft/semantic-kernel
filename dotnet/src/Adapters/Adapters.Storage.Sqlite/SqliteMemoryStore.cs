...
namespace Microsoft.SemanticKernel.Adapters.Storage.Sqlite;
...
public class SqliteMemoryStore : IMemoryStore, IDisposable
{
        ...
        CancellationToken cancel = default)
    {
        ...
        await memoryStore._dbConnection.OpenAsync(cancel).ConfigureAwait(false);
        await memoryStore._dbClient.CreateTableAsync(memoryStore._dbConnection, cancel).ConfigureAwait(false);
        return memoryStore;
    }
    ...
    {
        await this._dbClient.CreateCollectionAsync(this._dbConnection, collectionName, cancel).ConfigureAwait(false);
    }

    /// <inheritdoc/>
    public async Task<bool> DoesCollectionExistAsync(string collectionName, CancellationToken cancel = default)
    {
        return await this._dbClient.DoesCollectionExistsAsync(this._dbConnection, collectionName, cancel).ConfigureAwait(false);
    }

    /// <inheritdoc/>
    public async IAsyncEnumerable<string> GetCollectionsAsync([EnumeratorCancellation] CancellationToken cancel = default)
    {
        await foreach (var collection in this._dbClient.GetCollectionsAsync(this._dbConnection, cancel))
        {
    ...
    {
        await this._dbClient.DeleteCollectionAsync(this._dbConnection, collectionName, cancel).ConfigureAwait(false);
    }

    /// <inheritdoc/>
    public async Task<string> UpsertAsync(string collectionName, MemoryRecord record, CancellationToken cancel = default)
    {
        return await this.InternalUpsertAsync(this._dbConnection, collectionName, record, cancel).ConfigureAwait(false);
    }
        ...
        [EnumeratorCancellation] CancellationToken cancel = default)
    {
        ...
        {
            yield return await this.InternalUpsertAsync(this._dbConnection, collectionName, record, cancel).ConfigureAwait(false);
        }
    }
    ...
    {
        return await this.InternalGetAsync(this._dbConnection, collectionName, key, withEmbedding, cancel).ConfigureAwait(false);
    }
        ...
        [EnumeratorCancellation] CancellationToken cancel = default)
    {
        ...
        {
            var result = await this.InternalGetAsync(this._dbConnection, collectionName, key, withEmbeddings, cancel).ConfigureAwait(false);
            if (result != null)
            {
                yield return result;
            }
    ...
    {
        await this._dbClient.DeleteAsync(this._dbConnection, collectionName, key, cancel).ConfigureAwait(false);
    }

    /// <inheritdoc/>
    public async Task RemoveBatchAsync(string collectionName, IEnumerable<string> keys, CancellationToken cancel = default)
    {
        await Task.WhenAll(keys.Select(k => this._dbClient.DeleteAsync(this._dbConnection, collectionName, k, cancel))).ConfigureAwait(false);
    }
    ...
    private async IAsyncEnumerable<MemoryRecord> GetAllAsync(string collectionName, [EnumeratorCancellation] CancellationToken cancel = default)
    {
        ...
        await this._dbClient.DeleteEmptyAsync(this._dbConnection, collectionName, cancel).ConfigureAwait(false);

        await foreach (DatabaseEntry dbEntry in this._dbClient.ReadAllAsync(this._dbConnection, collectionName, cancel))
        {
    ...
    private async Task<string> InternalUpsertAsync(SqliteConnection connection, string collectionName, MemoryRecord record, CancellationToken cancel)
    {
        ...
        // Update
        await this._dbClient.UpdateAsync(
            conn: connection,
        ...
        await this._dbClient.InsertOrIgnoreAsync(
            conn: connection,
            collection: collectionName,
            key: record.Key,
            metadata: record.GetSerializedMetadata(),
    ...
    {
        DatabaseEntry? entry = await this._dbClient.ReadAsync(connection, collectionName, key, cancel).ConfigureAwait(false);
