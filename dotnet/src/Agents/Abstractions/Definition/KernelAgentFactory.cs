...
public abstract class KernelAgentFactory
{
    ...
    public async Task<KernelAgent> CreateAsync(Kernel kernel, AgentDefinition agentDefinition, IPromptTemplateFactory? templateFactory = null, CancellationToken cancellationToken = default)
    {
        Verify.NotNull(kernel);
        Verify.NotNull(agentDefinition);

        var kernelAgent = await this.TryCreateAsync(kernel, agentDefinition, templateFactory, cancellationToken).ConfigureAwait(false);
    ...
    /// <param name="cancellationToken">Optional cancellation token.</param>
    /// <return>The created <see cref="KernelAgent"/>, if null the agent type is not supported.</return>
    public abstract Task<KernelAgent?> TryCreateAsync(Kernel kernel, AgentDefinition agentDefinition, IPromptTemplateFactory? templateFactory = null, CancellationToken cancellationToken = default);
}
