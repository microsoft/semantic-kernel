...
public sealed class HuggingFaceClient : ITextCompletionClient, IEmbeddingGenerator<string, float>, IDisposable
{
    public HuggingFaceClient(Uri baseUri, string model)
    {
        this._model = model;
    ...
    public HuggingFaceClient(string baseUri, string model)
    ...
    private async Task<string> ExecuteCompleteRequestAsync(string text)
    {
        try
        {
            ...
            using var httpRequestMessage = new HttpRequestMessage()
            {
                ...
                RequestUri = new Uri(CompletionEndpoint),
    ...
    private async Task<IList<Embedding<float>>> ExecuteEmbeddingRequestAsync(IList<string> data)
    {
        try
        {
            ...
            {
                Method = HttpMethod.Post,
                RequestUri = new Uri(EmbeddingEndpoint),
