...
public class AzureAISearchVectorStoreCollectionSearchMappingTests
{
    ...
    {
        // Arrange.
        var filter = new BasicVectorSearchFilter().Equality(fieldName, fieldValue!);
    ...
    {
        // Arrange.
        var filter = new BasicVectorSearchFilter().TagListContains("Tags", "mytag");
    ...
    {
        // Arrange.
        var filter = new BasicVectorSearchFilter().Equality("intField", 5).TagListContains("Tags", "mytag");
    ...
    {
        // Act and assert.
        Assert.Throws<InvalidOperationException>(() => AzureAISearchVectorStoreCollectionSearchMapping.BuildFilterString(new BasicVectorSearchFilter().Equality("unknown", "value"), new Dictionary<string, string>()));
        Assert.Throws<InvalidOperationException>(() => AzureAISearchVectorStoreCollectionSearchMapping.BuildFilterString(new BasicVectorSearchFilter().TagListContains("unknown", "value"), new Dictionary<string, string>()));
    }
