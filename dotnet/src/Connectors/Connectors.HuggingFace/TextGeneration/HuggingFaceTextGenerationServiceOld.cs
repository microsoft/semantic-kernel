...
public sealed class HuggingFaceTextGenerationServiceOld : ITextGenerationService
#pragma warning restore CA1001 // Types that own disposable fields should be disposable. No need to dispose the Http client here. It can either be an internal client using NonDisposableHttpClientHandler or an external client managed by the calling code, which should handle its disposal.
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="HuggingFaceTextGenerationServiceOld"/> class.
    /// Using default <see cref="HttpClientHandler"/> implementation.
    /// </summary>
    /// <param name="endpoint">Endpoint for service API call.</param>
    /// <param name="model">Model to use for service API call.</param>
    public HuggingFaceTextGenerationServiceOld(Uri endpoint, string model)
    {
    ...
    public HuggingFaceTextGenerationServiceOld(string model, string? apiKey = null, HttpClient? httpClient = null, string? endpoint = null)
    {
        Verify.NotNullOrWhiteSpace(model);
