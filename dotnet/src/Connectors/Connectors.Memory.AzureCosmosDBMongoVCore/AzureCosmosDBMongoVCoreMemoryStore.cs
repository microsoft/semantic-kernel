...
public class AzureCosmosDBMongoVCoreMemoryStore: IMemoryStore, IDisposable
{
    ...
    {
        this._cosmosMongoDatabase.CreateCollectionAsync(collectionName, cancellationToken: cancellationToken);
        var indexes = await this.GetCollection(collectionName).Indexes.ListAsync().ConfigureAwait(false);
    ...
    {
        using var cursor = await this._cosmosMongoDatabase.ListCollectionNamesAsync(cancellationToken: cancellationToken).ConfigureAwait(false);
    ...
    public async Task<string> UpsertAsync(string collectionName, MemoryRecord record, CancellationToken cancellationToken = default)
    {
        ...
        var result = await GetCollection(collectionName)
            .ReplaceOneAsync(GetFilterById(record.Metadata.Id), new AzureCosmosDBMongoVCoreMemoryRecord(record), replaceOptions, cancellationToken)
    ...
    public async Task<MemoryRecord?> GetAsync(string collectionName, string key, bool withEmbedding = false, CancellationToken cancellationToken = default)
    {
        ...
        using var cursor = await this.GetCollection(collectionName).FindAsync(GetFilterById(key), null, cancellationToken).ConfigureAwait(false);

        var cosmosRecord = await cursor.SingleOrDefaultAsync(cancellationToken).ConfigureAwait(false);
    ...
    {
        using var cursor = await this.GetCollection(collectionName).FindAsync(GetFilterByIds(keys), null, cancellationToken).ConfigureAwait(false);
    ...
    {
        using var cursor = await this.VectorSearch(1, embedding, collectionName, cancellationToken).ConfigureAwait(false);
        ...
        MemoryRecord memoryRecord = AzureCosmosDBMongoVCoreMemoryRecord.ToMemoryRecord(result["document"].AsBsonDocument, withEmbedding);
    ...
    public async IAsyncEnumerable<(MemoryRecord, double)> GetNearestMatchesAsync(
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        using var cursor = await this.VectorSearch(limit, embedding, collectionName, cancellationToken).ConfigureAwait(false);
        while (await cursor.MoveNextAsync(cancellationToken).ConfigureAwait(false))
        {
            ...
            {   
                ...
                if (similarityScore < minRelevanceScore) { continue; }

                MemoryRecord memoryRecord = AzureCosmosDBMongoVCoreMemoryRecord.ToMemoryRecord(doc["document"].AsBsonDocument, withEmbeddings);
    ...
    private async Task<IAsyncCursor<BsonDocument>> VectorSearch(int limit, ReadOnlyMemory<float> embedding, string collectionName, CancellationToken cancellationToken)
    {
        ...
        using var cursor = await this.GetCollection(collectionName).AggregateAsync<BsonDocument>(pipeline).ConfigureAwait(false);
        return cursor;
    }
    ...
    private BsonDocument[] GetVectorIVFSearchPipeline(ReadOnlyMemory<float> embedding, int limit)
    {
        string searchStage = @"
        {
            ""$search"": {
                ""cosmosSearch"": {
                    ""vector"": [" + string.Join(",",  embedding.ToArray().Select(f => f.ToString())) + @"],
                    ""path"": ""embedding"",
                    ""k"": " + limit + @"
                },
    ...
    private IMongoCollection<AzureCosmosDBMongoVCoreMemoryRecord> GetCollection(string collectionName) =>
