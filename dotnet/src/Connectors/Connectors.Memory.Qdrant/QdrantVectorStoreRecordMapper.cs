    ...
    : IVectorStoreRecordMapper<TRecord, PointStruct>
{
    ...
    public PointStruct MapFromDataToStorageModel(TRecord dataModel)
    {
        ...
        var pointId = keyProperty.ClrType switch
        {
            ...
            _ => throw new VectorStoreRecordMappingException($"Unsupported key type '{keyProperty.ClrType.Name}' for key property '{keyProperty.ModelName}' on provided record of type '{typeof(TRecord).Name}'.")
        };
    ...
    {
        var outputRecord = model.InstantiateRecord<TRecord>()!;
        ...
        foreach (var dataProperty in model.DataProperties)
        {
            if (payload.TryGetValue(dataProperty.StorageName, out var fieldValue))
            {
                dataProperty.SetValueAsObject(
                    outputRecord,
                    QdrantVectorStoreRecordFieldMapping.ConvertFromGrpcFieldValueToNativeType(fieldValue, dataProperty.ClrType));
            }
        }
