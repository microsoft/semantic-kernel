...
This connector uses Redis to implement Semantic Memory. It requires the [RediSearch](https://github.com/RediSearch/redisearch-getting-started) module to be enabled on Redis to implement vector similarity search.

## What is RediSearch?

[RediSearch](https://github.com/RediSearch/redisearch-getting-started) is a source-available Redis module that enables querying, secondary indexing, and full-text search for Redis. These features enable multi-field queries, aggregation, exact phrase matching, numeric filtering, geo filtering and vector similarity semantic search on top of text queries.
...
1. Set up the RediSearch on your self-managed Redis, please refer to its [documentation](https://github.com/RediSearch/redisearch-getting-started).
