    ...
    : IVectorStoreRecordMapper<TConsumerDataModel, (string Key, HashEntry[] HashEntries)>
{
    ...
    public TConsumerDataModel MapFromStorageToDataModel((string Key, HashEntry[] HashEntries) storageModel, StorageToDataModelMapperOptions options)
    {
        ...
        var outputRecord = model.InstantiateRecord<TConsumerDataModel>();
        ...
        if (options?.IncludeVectors is true)
        {
            foreach (var property in model.VectorProperties)
            {
                ...
                {
                    property.SetValueAsObject(outputRecord!, property.ClrType switch
                    {
                        ...
                        _ => throw new VectorStoreRecordMappingException($"Unsupported vector type '{property.ClrType}'. Only float and double vectors are supported.")
                    });
                }
            }
        }
        ...
        foreach (var property in model.DataProperties)
        {
            ...
            {
                var typeOrNullableType = Nullable.GetUnderlyingType(property.ClrType) ?? property.ClrType;
