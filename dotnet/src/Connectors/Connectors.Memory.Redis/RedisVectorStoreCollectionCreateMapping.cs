...
internal static class RedisVectorStoreCollectionCreateMapping
{
    ...
    public static Schema MapToSchema(IEnumerable<VectorStoreRecordPropertyModel> properties, bool useDollarPrefix)
    {
        ...
        foreach (var property in properties)
        {
            ...
            switch (property)
            {
                ...
                case VectorStoreRecordDataPropertyModel dataProperty when dataProperty.IsFilterable || dataProperty.IsFullTextSearchable:
                    ...
                    {
                        if (dataProperty.ClrType == typeof(string) || (typeof(IEnumerable).IsAssignableFrom(dataProperty.ClrType) && GetEnumerableType(dataProperty.ClrType) == typeof(string)))
                        {
                    ...
                    {
                        if (dataProperty.ClrType == typeof(string))
                        {
                            schema.AddTagField(new FieldName($"{fieldNamePrefix}{storageName}", storageName));
                        }
                        else if (typeof(IEnumerable).IsAssignableFrom(dataProperty.ClrType) && GetEnumerableType(dataProperty.ClrType) == typeof(string))
                        {
                            schema.AddTagField(new FieldName($"{fieldNamePrefix}{storageName}.*", storageName));
                        }
                        else if (RedisVectorStoreCollectionCreateMapping.s_supportedFilterableNumericDataTypes.Contains(dataProperty.ClrType))
                        {
                            schema.AddNumericField(new FieldName($"{fieldNamePrefix}{storageName}", storageName));
                        }
                        else
                        {
                            throw new InvalidOperationException($"Property '{dataProperty.ModelName}' is marked as {nameof(VectorStoreRecordDataProperty.IsFilterable)}, but the property type '{dataProperty.ClrType}' is not supported. Only string, IEnumerable<string> and numeric properties are supported for filtering by the Redis VectorStore.");
                        }
                    }
    ...
    public static string GetSDKVectorType(VectorStoreRecordVectorPropertyModel vectorProperty)
        => vectorProperty.ClrType switch
        {
            Type t when t == typeof(ReadOnlyMemory<float>) => "FLOAT32",
            Type t when t == typeof(ReadOnlyMemory<float>?) => "FLOAT32",
            Type t when t == typeof(ReadOnlyMemory<double>) => "FLOAT64",
            Type t when t == typeof(ReadOnlyMemory<double>?) => "FLOAT64",
            _ => throw new InvalidOperationException($"Vector data type '{vectorProperty.ClrType.Name}' for {nameof(VectorStoreRecordVectorProperty)} '{vectorProperty.ModelName}' is not supported by the Redis VectorStore.")
        };
