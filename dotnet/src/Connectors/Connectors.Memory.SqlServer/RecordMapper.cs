...
internal sealed class RecordMapper<TRecord>(VectorStoreRecordModel model) : IVectorStoreRecordMapper<TRecord, IDictionary<string, object?>>
{
    ...
    {
        var record = model.InstantiateRecord<TRecord>()!;
        ...
        static void SetValue(IDictionary<string, object?> storageModel, object record, VectorStoreRecordPropertyModel property, object? value)
        {
            ...
            {
                throw new VectorStoreRecordMappingException($"Failed to set value '{value}' on property '{property.ModelName}' of type '{property.ClrType.Name}'.", ex);
            }
        }
    }
}
