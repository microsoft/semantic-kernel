...
internal static class SqlServerCommandBuilder
{
    ...
    private static void AddParameter(this SqlCommand command, VectorStoreRecordPropertyModel property, string name, object? value)
    {
        switch (value)
        {
            case null when property.ClrType == typeof(byte[]):
    ...
    private static string Map(VectorStoreRecordPropertyModel property) => property.ClrType switch
    {
        ...
        _ => throw new NotSupportedException($"Type {property.ClrType} is not supported.")
    };
