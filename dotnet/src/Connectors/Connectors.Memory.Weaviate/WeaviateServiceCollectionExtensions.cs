...
/// <summary>
/// Extension methods to register Weaviate <see cref="VectorStore"/> instances on an <see cref="IServiceCollection"/>
/// </summary>
public static class WeaviateServiceCollectionExtensions
{
    /// <summary>
    /// Register a Weaviate <see cref="VectorStore"/> with the specified service ID.
    /// </summary>
    ...
    private static void AddVectorizedSearch<TRecord>(IServiceCollection services, string? serviceId) where TRecord : notnull
    {
        services.AddKeyedTransient<IVectorSearch<TRecord>>(
            ...
            (sp, obj) =>
            {
                return sp.GetRequiredKeyedService<VectorStoreCollection<Guid, TRecord>>(serviceId);
            });
    }
}
