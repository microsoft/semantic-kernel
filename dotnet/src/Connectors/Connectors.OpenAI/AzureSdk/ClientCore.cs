        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        for (int iteration = 1; ; iteration++)
        {
            ...
            await foreach (StreamingChatCompletionsUpdate update in response.ConfigureAwait(false))
            {
                ...
                yield return new OpenAIStreamingChatMessageContent(update, update.ChoiceIndex ?? 0, this.DeploymentOrModelName, metadata) { Name = streamedName };
            }
...
#pragma warning disable SKEXP0011 // Assigning nullable property self-consistent for internal/private code.
            chat.Add(new OpenAIChatMessageContent(streamedName, streamedRole ?? default, content, this.DeploymentOrModelName, toolCalls, metadata));
