...
public sealed class AzureOpenAITextToAudioService : ITextToAudioService
{
    ...
    public Task<AudioContent> GetAudioContentAsync(string text, PromptExecutionSettings? executionSettings = null, Kernel? kernel = null, CancellationToken cancellationToken = default)
        => this.InternalGetAudioContentAsync(text, executionSettings, kernel, cancellationToken);

    #region private

    private async Task<AudioContent> InternalGetAudioContentAsync(string text, PromptExecutionSettings? executionSettings = null, Kernel? kernel = null, CancellationToken cancellationToken = default)
    {
    ...
    private string GetRequestUri() => $"{this._endpoint.TrimEnd('/')}/openai/deployments/{this._deploymentName}/audio/speech?api-version={DefaultApiVersion}";
