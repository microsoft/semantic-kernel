...
public abstract class ToolCallBehavior
{
    ...
    internal sealed class KernelFunctions : ToolCallBehavior
    {
        ...
        internal override void ConfigureOptions(Kernel? kernel, ChatCompletionOptions options)
        {
            ...
            if (kernel is not null)
            {
                ...
                {
                    options = options.CloneWith(ChatToolChoice.Auto);
                    for (int i = 0; i < functions.Count; i++)
                    {
                        options.Tools.Add(functions[i].ToOpenAIFunction().ToFunctionDefinition());
                    }
                }
            }
        }
    ...
    internal sealed class RequiredFunction : ToolCallBehavior
    {
        ...
        public RequiredFunction(OpenAIFunction function, bool autoInvoke) : base(autoInvoke)
        {
            ...
            this._choice = ChatToolChoice.Required;
        }
        ...
        internal override void ConfigureOptions(Kernel? kernel, ChatCompletionOptions options)
        {
            ...
            options = options.CloneWith(this._choice);
