...
public class QdrantVectorStoreRecordCollectionTests
{
        ...
        where TKey : notnull
    {
        ...
        var filter = new BasicVectorSearchFilter().Equality(nameof(SinglePropsModel<TKey>.Data), "data 1");

        // Act.
        var actual = await sut!.SearchAsync(
            VectorSearchQuery.CreateQuery(new ReadOnlyMemory<float>(new[] { 1f, 2f, 3f, 4f }), new() { IncludeVectors = true, BasicVectorSearchFilter = filter, Limit = 5, Offset = 2 }),
            this._testCancellationToken).ToListAsync();
