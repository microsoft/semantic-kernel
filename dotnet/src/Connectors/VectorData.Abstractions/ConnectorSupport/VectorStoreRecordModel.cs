...
public sealed class VectorStoreRecordModel
{
    ...
    private readonly Type _recordClrType;
    ...
    {
        this._recordClrType = recordClrType;
    ...
    // populated.
    public TRecord InstantiateRecord<TRecord>()
    {
        Debug.Assert(typeof(TRecord) == this._recordClrType, "Type mismatch between record type and model type.");

        return Activator.CreateInstance<TRecord>() ?? throw new InvalidOperationException("Failed to instantiate record.");
    }
...
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            if (!string.IsNullOrWhiteSpace(vectorPropertyName))
            {
                ...
                return this.VectorProperties.FirstOrDefault(p => p.ModelName == vectorPropertyName)
                    ?? throw new InvalidOperationException($"The {this._recordClrType.FullName} type does not have a vector property named '{vectorPropertyName}'.");
            }
        ...
        return this._singleVectorProperty ??= this.VectorProperties switch
        {
            ...
            { Count: 0 } => throw new InvalidOperationException($"The '{this._recordClrType.Name}' type does not have any vector properties."),
            _ => throw new InvalidOperationException($"The '{this._recordClrType.Name}' type has multiple vector properties, please specify your chosen property via options.")
        };
    }
    ...
    public VectorStoreRecordDataPropertyModel GetFullTextDataPropertyOrSingle<TRecord>(Expression<Func<TRecord, object?>>? expression)
    {
        if (expression is not null)
        {
            ...
            return property.IsFullTextSearchable
                ? property
                : throw new InvalidOperationException($"The property '{property.ModelName}' on '{this._recordClrType.Name}' must have full text search enabled.");
        }
        ...
        if (this._singleFullTextSearchProperty is null)
        {
            ...
            var fullTextStringProperties = this.DataProperties
                .Where(l => l.ClrType == typeof(string) && l.IsFullTextSearchable)
                .ToList();
            ...
            {
                [var singleProperty] => singleProperty,
                { Count: 0 } => throw new InvalidOperationException($"The '{this._recordClrType.Name}' type does not have any text data properties that have full text search enabled."),
                _ => throw new InvalidOperationException($"The '{this._recordClrType.Name}' type has multiple text data properties that have full text search enabled, please specify your chosen property via options.")
            };
        }
        ...
        where TProperty : VectorStoreRecordPropertyModel
    {
            ...
            && member.Expression == expression.Parameters[0])
        {
            ...
            {
                if (property.ClrProperty == clrProperty)
                {
