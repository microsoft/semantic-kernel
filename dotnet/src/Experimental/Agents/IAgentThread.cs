...
public interface IAgentThread
{
    ...
    /// <returns></returns>
    Task<IChatMessage> AddUserMessageAsync(string message, string[]? fileIds = null, CancellationToken cancellationToken = default);
    ...
    /// <param name="cancellationToken">A cancellation token</param>
    /// <returns>The resulting agent message(s)</returns>
    IAsyncEnumerable<IChatMessage> InvokeAsync(IAgent agent, string userMessage, KernelArguments? arguments = null, string[]? fileIds = null, CancellationToken cancellationToken = default);
