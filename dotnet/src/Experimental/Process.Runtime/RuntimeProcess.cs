...
internal sealed class RuntimeProcess : RuntimeStep, IDisposable
{
    ...
    internal readonly List<RuntimeStep> _steps = [];
    ...
    internal RuntimeProcess(KernelProcess process, Kernel kernel, IAgentRuntime runtime)
        : base(process, kernel, runtime)
    {
    ...
    private async ValueTask InitializeProcessAsync()
    {
        ...
        foreach (var stepInfo in this._stepsInfos)
        {
            RuntimeStep? step = null;

            // The current step should already have a name.
            Verify.NotNull(stepInfo.State?.Name);
            ...
            if (stepInfo is KernelProcess processStep)
            {
                ...
                step =
                    new RuntimeProcess(processStep, this._kernel, this.Runtime)
                    {
            ...
            else
            {
                ...
                step =
                    new RuntimeStep(stepInfo, this._kernel, this.Runtime)
                    {
                        ParentProcessId = this.StepId,
                        EventProxy = this.EventProxy,
    ...
    /// <param name="messageChannel">The message channel where messages should be enqueued.</param>
    private void EnqueueStepMessages(RuntimeStep step, Queue<ProcessMessage> messageChannel)
    {
    ...
    {
        var processState = new KernelProcessState(this.Name, this._stepState.Version, this.StepId);
