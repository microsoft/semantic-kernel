...
/// </summary>
internal class RuntimeStep : BaseAgent, IKernelProcessMessageChannel, IHandle<ProcessMessage>
{
    ...
    public RuntimeStep(KernelProcessStepInfo stepInfo, Kernel kernel, IAgentRuntime runtime, string? parentProcessId = null)
        : base(
            id: new AgentId(FormatAgentType(stepInfo.State.Id!), stepInfo.State.Id!), // %%% HACKING AgentType in conjunction RegisterAgentFactoryAsync HACK
            runtime,
            description: GenerateDescription(stepInfo),
            logger: kernel.LoggerFactory?.CreateLogger<BaseAgent>() ?? new NullLogger<BaseAgent>()) // %%% T !== BaseAgent
    {
        Verify.NotNull(kernel, nameof(kernel));
        Verify.NotNull(stepInfo, nameof(stepInfo));

        // %%% - WHEN? This special handling will be removed with the refactoring of KernelProcessState
    ...
    private const string DefaultDescriptionPrefix = "RuntimeStep";

    private static readonly Dictionary<Type, string> s_descriptionPrefixMap =
        new()
        {
            {typeof(KernelProcess), "RuntimeProcess"},
            {typeof(KernelProcessMap), "RuntimeMap"},
            {typeof(KernelProcessStepInfo), DefaultDescriptionPrefix},
        };

    private static string GenerateDescription(KernelProcessStepInfo stepInfo)
    {
        if (!s_descriptionPrefixMap.TryGetValue(stepInfo.GetType(), out string? prefix))
        {
            prefix = DefaultDescriptionPrefix;
        }

        return $"{prefix} {stepInfo.State.Name}";
    }
    ...
    /// The Id of the step.
    /// </summary>
    internal string StepId => this._stepInfo.State.Id!;
    ...
    /// <inheritdoc/>
    public async ValueTask HandleAsync(ProcessMessage item, MessageContext messageContext)
    {
        await this.HandleMessageAsync(item).ConfigureAwait(false);
    }
...
#pragma warning restore CA1031 // Do not catch general exception types
    ...
    protected ProcessEvent ScopedEvent(ProcessEvent localEvent)
    {
        ...
        return localEvent with { Namespace = $"{this.Name}_{this.StepId}" };
    }

    protected static string FormatAgentType(string id) => $"T{id.Replace("-", "_")}"; // %%% HACK
}
