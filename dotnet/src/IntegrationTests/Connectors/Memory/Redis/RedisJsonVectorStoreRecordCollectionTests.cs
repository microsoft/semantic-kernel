...
public sealed class RedisJsonVectorStoreRecordCollectionTests(ITestOutputHelper output, RedisVectorStoreFixture fixture)
{
    ...
    public async Task ItCanCreateACollectionUpsertGetAndSearchAsync(bool useRecordDefinition)
    {
        ...
        var searchResult = await sut.SearchAsync(VectorSearchQuery.CreateQuery(new ReadOnlyMemory<float>(new[] { 30f, 31f, 32f, 33f }), new VectorSearchOptions { BasicVectorSearchFilter = new BasicVectorSearchFilter().Equality("HotelCode", 10) })).ToListAsync();
    ...
    public async Task ItCanSearchWithVectorAndFilterAsync(string filterType)
    {
        ...
        var filter = filterType == "equality" ? new BasicVectorSearchFilter().Equality("HotelCode", 1) : new BasicVectorSearchFilter().TagListContains("Tags", "pool");

        // Act
        var actual = await sut.SearchAsync(
            VectorSearchQuery.CreateQuery(
                vector,
                new VectorSearchOptions { IncludeVectors = true, BasicVectorSearchFilter = filter }))
            .ToListAsync();
