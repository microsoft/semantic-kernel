...
public sealed class ProcessCycleTests
{
    ...
    public async Task TestCycleAndExitWithFanInAsync()
    {
        ...
        var processContext = await kernelProcess.StartAsync(kernel, new KernelProcessEvent<string>() { Id = CommonEvents.StartProcess, Data = "foo" });
...
#pragma warning disable CA1812 // Avoid uninstantiated internal classes
    ...
    private sealed class KickoffStep : KernelProcessStep
    {
        ...
        {
            await context.EmitEventAsync(new KernelProcessEvent<string>() { Id = CommonEvents.StartARequested, Data = "Get Going A" });
            await context.EmitEventAsync(new KernelProcessEvent<string>() { Id = CommonEvents.StartBRequested, Data = "Get Going B" });
        }
    }
    ...
    private sealed class AStep : KernelProcessStep
    {
        ...
        public async ValueTask DoItAsync(KernelProcessStepContext context)
        {
            ...
            await context.EmitEventAsync(new KernelProcessEvent<string>() { Id = CommonEvents.AStepDone, Data = "I did A" });
        }
    }
    ...
    private sealed class BStep : KernelProcessStep
    {
        ...
        public async ValueTask DoItAsync(KernelProcessStepContext context)
        {
            ...
            await context.EmitEventAsync(new KernelProcessEvent<string>() { Id = CommonEvents.BStepDone, Data = "I did B" });
        }
    }
