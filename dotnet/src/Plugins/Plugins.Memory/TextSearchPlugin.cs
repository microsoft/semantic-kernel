...
public sealed class TextSearchPlugin<T>(ITextSearch<T> service) where T : class
{
    private readonly ITextSearch<T> _service = service;
    ...
    {
        var results = await this._service.SearchAsync(query, new() { Count = count, Offset = offset }, cancellationToken).ConfigureAwait(false);
