...
public static class ChatCompletionExtensions
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            return chatCompletion.GetChatMessageContentsAsync(chatHistory, executionSettings, kernel, cancellationToken);
        }

        //Otherwise, use the prompt as the chat system message
        return chatCompletion.GetChatMessageContentsAsync(new ChatHistory(prompt), executionSettings, kernel, cancellationToken);
    }
    ...
    public static async Task<ChatMessageContent> GetChatMessageContentAsync(
        ...
        Kernel? kernel = null,
        CancellationToken cancellationToken = default)
        => (await chatCompletion.GetChatMessageContentsAsync(prompt, executionSettings, kernel, cancellationToken).ConfigureAwait(false))
            .Single();
    ...
    public static async Task<ChatMessageContent> GetChatMessageContentAsync(
        ...
        Kernel? kernel = null,
        CancellationToken cancellationToken = default)
        => (await chatCompletion.GetChatMessageContentsAsync(chatHistory, executionSettings, kernel, cancellationToken).ConfigureAwait(false))
            .Single();

    /// <summary>
    /// Get streaming chat message contents for the chat history provided using the specified request settings.
    /// </summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (XmlPromptParser.TryParse(prompt!, out var nodes) && ChatPromptParser.TryParse(nodes, out var chatHistory))
        {
            return chatCompletion.GetStreamingChatMessageContentsAsync(chatHistory, executionSettings, kernel, cancellationToken);
        }

        //Otherwise, use the prompt as the chat system message
        return chatCompletion.GetStreamingChatMessageContentsAsync(new ChatHistory(prompt), executionSettings, kernel, cancellationToken);
    }
}
