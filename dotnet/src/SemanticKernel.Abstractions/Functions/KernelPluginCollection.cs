...
/// <summary>Provides a collection of <see cref="ReadOnlyKernelPlugin"/>s.</summary>
/// <remarks>
...
public sealed class KernelPluginCollection : ICollection<ReadOnlyKernelPlugin>, IReadOnlyKernelPluginCollection
{
    /// <summary>The underlying dictionary of plugins.</summary>
    private readonly Dictionary<string, ReadOnlyKernelPlugin> _plugins;
    ...
    public KernelPluginCollection(IEnumerable<ReadOnlyKernelPlugin> plugins)
    {
        ...
        {
            this._plugins = new(plugins is ICollection<ReadOnlyKernelPlugin> c ? c.Count : 0, StringComparer.OrdinalIgnoreCase);
    ...
    /// <exception cref="ArgumentNullException"><paramref name="plugin"/>.<see cref="ReadOnlyKernelPlugin.Name"/> is null.</exception>
    /// <exception cref="ArgumentException">A plugin with the same name already exists in the collection.</exception>
    public void Add(ReadOnlyKernelPlugin plugin)
    {
    ...
    /// <exception cref="ArgumentNullException">A plugin in <paramref name="plugins"/> has a null <see cref="ReadOnlyKernelPlugin.Name"/>.</exception>
    ...
    public void AddRange(IEnumerable<ReadOnlyKernelPlugin> plugins)
    {
        Verify.NotNull(plugins);
    ...
    public bool Remove(ReadOnlyKernelPlugin plugin)
    {
        Verify.NotNull(plugin);

        if (this._plugins.TryGetValue(plugin.Name, out ReadOnlyKernelPlugin? existing) && existing == plugin)
        {
    ...
    public IEnumerator<ReadOnlyKernelPlugin> GetEnumerator() => this._plugins.Values.GetEnumerator();
    ...
    public bool Contains(ReadOnlyKernelPlugin plugin)
    {
        Verify.NotNull(plugin);

        return this._plugins.TryGetValue(plugin.Name, out ReadOnlyKernelPlugin? existing) && plugin == existing;
    }

    /// <inheritdoc/>
    public ReadOnlyKernelPlugin this[string name]
    {
        get
        {
            if (!this.TryGetPlugin(name, out ReadOnlyKernelPlugin? plugin))
            {
    ...
    /// <returns>true if the collection contains the plugin; otherwise, false.</returns>
    public bool TryGetPlugin(string name, [NotNullWhen(true)] out ReadOnlyKernelPlugin? plugin) =>
        this._plugins.TryGetValue(name, out plugin);

    void ICollection<ReadOnlyKernelPlugin>.CopyTo(ReadOnlyKernelPlugin[] array, int arrayIndex) =>
        ((IDictionary<string, ReadOnlyKernelPlugin>)this._plugins).Values.CopyTo(array, arrayIndex);

    bool ICollection<ReadOnlyKernelPlugin>.IsReadOnly => false;
    ...
    private sealed class TypeProxy
    {
        ...
        public ReadOnlyKernelPlugin[] Plugins => this._collection._plugins.Values.ToArray();
    }
}
