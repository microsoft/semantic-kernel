...
public sealed class SKPluginCollection : ISKPluginCollection
{
    ...
    public SKPluginCollection(IEnumerable<ISKPlugin> plugins)
    {
    ...
    /// <exception cref="ArgumentNullException"><paramref name="plugin"/>.<see cref="ISKPlugin.Name"/> is null.</exception>
    /// <exception cref="ArgumentException">A plugin with the same name already exists in the collection.</exception>
    public void Add(ISKPlugin plugin)
    {
    ...
    public bool Remove(ISKPlugin plugin)
    {
        Verify.NotNull(plugin);

        if (this._plugins.TryGetValue(plugin.Name, out ISKPlugin? existing) && existing == plugin)
        {
    ...
    public ISKPlugin this[string name]
    {
        get
        {
            if (!this.TryGetPlugin(name, out ISKPlugin? plugin))
            {
    ...
    {
        private readonly SKPluginCollection _collection;

        public TypeProxy(SKPluginCollection collection) => this._collection = collection;

        [DebuggerBrowsable(DebuggerBrowsableState.RootHidden)]
        public ISKPlugin[] Plugins => this._collection._plugins.Values.ToArray();
    }
}
