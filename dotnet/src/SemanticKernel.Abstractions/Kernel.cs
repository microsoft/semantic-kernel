...
public sealed class Kernel
{
        ...
        KernelPluginCollection? plugins = null)
    {
        ...
        if (this._plugins is null)
        {
            ...
            IEnumerable<ReadOnlyKernelPlugin> e = this.Services.GetServices<ReadOnlyKernelPlugin>();
            ...
            if (e is not ICollection<ReadOnlyKernelPlugin> c || c.Count != 0)
            {
                this._plugins = new(e);
            }
        }
    ...
    /// </item>
    /// <item>
    /// A new <see cref="KernelPluginCollection"/> instance initialized with the same <see cref="ReadOnlyKernelPlugin"/> instances as are stored by the current instance's <see cref="Kernel.Plugins"/> collection.
