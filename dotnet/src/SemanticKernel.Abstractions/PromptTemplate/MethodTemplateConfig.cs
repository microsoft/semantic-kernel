...
/// This can all be provided by a <see cref="MethodTemplateConfig"/>, where its <see cref="MethodTemplateConfig.Template"/>
...
/// this is done automatically by the APIs that accept a <see cref="MethodTemplateConfig"/>, using a default template
/// factory that understands the <see cref="MethodTemplateConfig.SemanticKernelTemplateFormat"/> format, but with the
/// ability to supply other factories for interpreting other formats.
/// </remarks>
public sealed class MethodTemplateConfig
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="MethodTemplateConfig"/> class.
    /// </summary>
    public MethodTemplateConfig()
    {
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="MethodTemplateConfig"/> class using the specified prompt template string.
    /// </summary>
    /// <param name="template">The prompt template string that defines the prompt.</param>
    /// <exception cref="ArgumentNullException"><paramref name="template"/> is null.</exception>
    public MethodTemplateConfig(string template)
    {
        this.Template = template;
    }

    /// <summary>
    /// Creates a <see cref="MethodTemplateConfig"/> from the specified JSON.
    /// </summary>
    /// <param name="json">A string containing a JSON representation of the <see cref="MethodTemplateConfig"/>.</param>
    /// <returns>The deserialized <see cref="MethodTemplateConfig"/>.</returns>
    /// <exception cref="ArgumentNullException"><paramref name="json"/> is null.</exception>
    /// <exception cref="ArgumentException"><paramref name="json"/> is an invalid JSON representation of a <see cref="MethodTemplateConfig"/>.</exception>
    public static MethodTemplateConfig FromJson(string json)
    {
        Verify.NotNullOrWhiteSpace(json);

        Exception? innerException = null;
        MethodTemplateConfig? config = null;
        try
        {
            config = JsonSerializer.Deserialize<MethodTemplateConfig>(json, JsonOptionsCache.ReadPermissive);
            if (config is null)
            {
                throw new ArgumentException($"Unable to deserialize {nameof(MethodTemplateConfig)} from the specified JSON.", nameof(json));
            }
        ...
        return
            config ??
            throw new ArgumentException($"Unable to deserialize {nameof(MethodTemplateConfig)} from the specified JSON.", nameof(json), innerException);
    }
