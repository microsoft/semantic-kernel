...
public sealed class Plan : ISKFunction
{
    ...
    public FunctionView Describe()
    {
        ...
        var parameters = this.Parameters.Select(p =>
        {
            ...
            return new ParameterView(p.Key, stepDescription?.Description, stepDescription?.DefaultValue, stepDescription?.Type, stepDescription?.IsRequired, stepDescription?.NativeType, stepDescription?.Schema);
        }
        ).ToList();
