...
public sealed class KernelPromptTemplate : IPromptTemplate
{
    ...
    internal async Task<string> RenderAsync(IList<Block> blocks, Kernel kernel, SKContext context, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var block in blocks)
        {
            switch (block)
            {
                ...
                case ICodeRendering dynamicBlock:
                    tasks.Add(dynamicBlock.RenderCodeAsync(kernel, context, cancellationToken));
                    break;
