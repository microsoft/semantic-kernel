...
// Eventually, the intent is that the Microsoft.Extensions.AI interfaces will be the primary interfaces used by Semantic Kernel,
// with helper extension methods providing the same general surface area as is provided on the SK interfaces today, e.g.
//     public static async Task<IReadOnlyList<ChatMessageContent>> GetChatMessageContentsAsync(
...
// In the meantime, the methods in this type provide bidirectional conversions between the two sets of interfaces.

/// <summary>
/// Provides extension methods for converting between Microsoft.SemanticKernel types and core .NET types.
/// </summary>
[Experimental("SKEXP0001")]
public static class ServiceConversionExtensions
{
        ...
        where TEmbedding : unmanaged
    {
        ...
        {
            // IEmbeddingGenerationService include telemetry, logging, etc.
            // IEmbeddingGenerator may not. Add in such capabilities as needed to fulfill the expectations.
            if (generator.GetService<IChatCompletionService>() is null)
            {
                if (ModelDiagnostics.s_enableDiagnostics &&
                    generator.GetService<OpenTelemetryEmbeddingGenerator<TValue, Embedding<TEmbedding>>>() is null)
                {
                    generator = new OpenTelemetryEmbeddingGenerator<TValue, Embedding<TEmbedding>>(generator, ModelDiagnostics.s_namespace);
                }

                if (generator.GetService<LoggingEmbeddingGenerator<TValue, Embedding<TEmbedding>>>() is null &&
                    serviceProvider?.GetService<ILogger<LoggingEmbeddingGenerator<TValue, Embedding<TEmbedding>>>>() is ILogger logger)
                {
                    generator = new LoggingEmbeddingGenerator<TValue, Embedding<TEmbedding>>(generator, logger);
                }
            }
    ...
    private sealed class ChatClientChatCompletionService : IChatCompletionService
    {
        ...
        public ChatClientChatCompletionService(IChatClient chatClient, IServiceProvider? serviceProvider)
        {
            ...
            // IChatCompletionServices include telemetry, logging, automatic function calling, etc.
            // IChatClients are more composable and may not. Heuristically add in such capabilities as
            // needed to fulfill the expectations.
            if (chatClient.GetService<IChatCompletionService>() is null)
            {
                if (ModelDiagnostics.s_enableDiagnostics &&
                    chatClient.GetService<OpenTelemetryChatClient>() is null)
                {
                    chatClient = new OpenTelemetryChatClient(chatClient, ModelDiagnostics.s_namespace)
                    {
                        EnableSensitiveData = ModelDiagnostics.s_enableSensitiveEvents,
                    };
                }

                if (chatClient.GetService<LoggingChatClient>() is null &&
                    serviceProvider?.GetService<ILogger<LoggingChatClient>>() is ILogger logger)
                {
    ...
    private static ChatOptions? ToChatOptions(PromptExecutionSettings? settings, Kernel? kernel)
    {
        ...
        if (settings is not null)
        {
            if (settings.GetType() != typeof(PromptExecutionSettings))
            {
                ...
                settings = JsonSerializer.Deserialize<PromptExecutionSettings>(JsonSerializer.Serialize(settings));
            }
            ...
            if (settings!.ExtensionData is IDictionary<string, object> extensionData)
            {
                foreach (var entry in extensionData)
                {
                        ...
                        entry.Value is int maxTokens)
                    {
                        ...
                        entry.Value is IList<string> stopSequences)
                    {
        ...
        if (options.ResponseFormat is not null)
        {
            ...
            else if (options.ResponseFormat is ChatResponseFormatJson json)
            {
                ...
                settings.ExtensionData["response_format"] = json.Schema is not null ?
                    JsonSerializer.Deserialize<JsonElement>(json.Schema) :
                    "json";
            }
        }
    ...
    private static ChatMessage ToChatMessage(ChatMessageContent content)
    {
        ...
        foreach (var item in content.Items)
        {
            ...
            {
                aiContent.ModelId = content.ModelId;
    ...
    private static ChatMessageContent ToChatMessageContent(ChatMessage message)
    {
        ...
        foreach (AIContent content in message.Contents)
        {
            ...
            {
                resultContent.ModelId = content.ModelId;
            ...
            switch (item)
            {
                ...
                case Microsoft.SemanticKernel.StreamingFunctionCallUpdateContent fcc:
                    aiContent = new Microsoft.Extensions.AI.FunctionCallContent(
                        ...
                        fcc.Arguments is not null ? JsonSerializer.Deserialize<IDictionary<string, object?>>(fcc.Arguments) : null);
                    break;
            }
    ...
    private static StreamingChatMessageContent ToStreamingChatMessageContent(StreamingChatCompletionUpdate update)
    {
        ...
        foreach (AIContent item in update.Contents)
        {
            StreamingKernelContent? resultContent =
                ...
                item is Microsoft.Extensions.AI.FunctionCallContent fcc ? new Microsoft.SemanticKernel.StreamingFunctionCallUpdateContent(fcc.CallId, fcc.Name, fcc.Arguments is not null ? JsonSerializer.Serialize(fcc.Arguments) : null) :
                null;

            if (resultContent is not null)
            {
                resultContent.ModelId = item.ModelId;
    ...
    private sealed class AIFunctionKernelFunction : KernelFunction
    {
        ...
        public AIFunctionKernelFunction(AIFunction aiFunction) :
            base(aiFunction.Metadata.Name,
                ...
                aiFunction.Metadata.Parameters.Select(p => new KernelParameterMetadata(p.Name)
                {
                    ...
                    Schema =
                        ...
                        p.Schema is string s ? new KernelJsonSchema(JsonSerializer.Deserialize<JsonElement>(s)) :
                        null,
                }).ToList(),
                new KernelReturnParameterMetadata()
                {
                    ...
                    Schema =
                        ...
                        aiFunction.Metadata.ReturnParameter.Schema is string s ? new KernelJsonSchema(JsonSerializer.Deserialize<JsonElement>(s)) :
                        null,
                })
        {
        ...
        private AIFunctionKernelFunction(AIFunctionKernelFunction other, string pluginName) :
            base(other.Name, pluginName, other.Description, other.Metadata.Parameters, other.Metadata.ReturnParameter)
        {
