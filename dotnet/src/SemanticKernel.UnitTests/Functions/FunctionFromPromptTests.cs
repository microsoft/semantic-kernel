...
public class FunctionFromMethodTests
{
    ...
    [Fact]
    public async Task InvokeStreamingAsyncShouldTriggerInvokeAndInvokedChunkFromNonStreamingMethodAsync()
    {
        // Arrange
        var kernel = new KernelBuilder().Build();
    ...
    [Fact]
    public async Task InvokeStreamingAsyncInvokedCancellingShouldRenderNoChunksAsync()
    {
        // Arrange
        var kernel = new KernelBuilder().Build();
        var sut = SKFunctionFactory.CreateFromMethod(() => "any");
