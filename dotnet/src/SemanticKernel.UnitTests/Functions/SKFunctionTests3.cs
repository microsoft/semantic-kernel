...
public sealed class SKFunctionTests3
{
    ...
    public void ItDoesntThrowForValidFunctionsViaDelegate()
    {
        ...
        SKFunction[] functions = (from method in methods select SKFunctionHelper.FromMethod(method, pluginInstance, "plugin")).ToArray();
    ...
    public void ItDoesNotThrowForValidFunctionsViaPlugin()
    {
        ...
        SKFunction[] functions = new KernelBuilder().Build().ImportPluginFromObject(pluginInstance).ToArray();
    ...
    public void ItThrowsForInvalidFunctions()
    {
        ...
        foreach (var method in methods)
        {
            try
            {
                SKFunctionHelper.FromMethod(method, instance, "plugin");
            }
            catch (SKException)
            {
                count++;
    ...
    public async Task ItCanImportNativeFunctionsAsync()
    {
        ...
        // Act
        SKFunction function = SKFunctionHelper.FromMethod(
            method: ExecuteAsync,
            parameters: null,
    ...
    public async Task ItCanImportNativeFunctionsWithExternalReferencesAsync()
    {
        ...
        // Act. Note: this will throw an exception if SKFunctionHelper doesn't handle the function type.
        SKFunction function = SKFunctionHelper.FromMethod(
            method: ExecuteAsync,
            description: "description",
            functionName: "functionName");
