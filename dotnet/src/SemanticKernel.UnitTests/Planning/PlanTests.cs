...
public sealed class PlanTests
{
    ...
    public async Task CanExecutePlanWithPlanStepAsync()
    {
        ...
        var function = SKFunctionHelper.FromMethod((SKContext context) =>
        {
    ...
    public async Task CanExecutePlanWithFunctionStepsAsync()
    {
        ...
        var function1 = SKFunctionHelper.FromMethod((SKContext context) =>
        {
            Assert.Equal(planInput, context.Variables.Input);
            return "fake result of function 1";
        }, "function1");

        var function2 = SKFunctionHelper.FromMethod((SKContext context) =>
        {
    ...
    public async Task StepExceptionIsThrownAsync()
    {
        ...
        var mockFunction = new SKFunctionMock
        {
            InvokeDelegate = (_, _, _, _) => throw new ArgumentException("Error message"),
            DescribeDelegate = () => new FunctionView("functionName")
        };
    ...
    public async Task CanExecutePlanWithTreeStepsAsync()
    {
        ...
        var childFunction1 = SKFunctionHelper.FromMethod((SKContext context) =>
        {
            return "Child 1 output!" + context.Variables.Input;
        },
        "childFunction1");

        var childFunction2 = SKFunctionHelper.FromMethod((SKContext context) =>
        {
            return "Child 2 is happy about " + context.Variables.Input;
        },
        "childFunction2");

        var childFunction3 = SKFunctionHelper.FromMethod((SKContext context) =>
        {
            return "Child 3 heard " + context.Variables.Input;
        },
        "childFunction3");

        var nodeFunction1 = SKFunctionHelper.FromMethod((SKContext context) =>
        {
...
Previously:Outline section #1 of 3: Here is a 3 chapter outline about NovelOutline function input.";
    ...
    public async Task ConPlanStepsTriggerKernelEventsAsync()
    {
        ...
        functions.Add(SKFunctionHelper.FromMethod(Method(Function2)));

        [SKName("SendEmail")]
        static string Function3() => "Sent Email";
        functions.Add(SKFunctionHelper.FromMethod(Method(Function3)));
