...
/// </summary>
public class SKFunctionMock : SKFunction
{
    public Func<FunctionView>? DescribeDelegate { get; set; }
    public Func<Kernel, SKContext, AIRequestSettings?, CancellationToken, Task<FunctionResult>>? InvokeDelegate { get; set; }

    public SKFunctionMock(string? name = null, string? description = null, IEnumerable<AIRequestSettings>? modelSettings = null) : base(name ?? string.Empty, description ?? string.Empty, modelSettings)
    {
    }

    public override FunctionView Describe()
    {
        if (this.DescribeDelegate is null)
        {
            return new FunctionView(this.Name);
        }

        return this.DescribeDelegate();
    }
