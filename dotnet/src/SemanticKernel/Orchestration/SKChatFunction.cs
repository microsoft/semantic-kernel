...
/// </summary>
public sealed class SKChatFunction : ISKChatFunction, IDisposable
{
    /// <inheritdoc/>
    ...
    public static ISKChatFunction? FromNativeMethod(
    ...
    /// <param name="chatfunctionConfig">Semantic function configuration.</param>
    /// <param name="log">Optional logger for the function.</param>
    /// <returns>SK function instance.</returns>
    public static ISKChatFunction FromSemanticConfig(
        string skillName,
        string functionName,
        SemanticChatFunctionConfig chatfunctionConfig,
        ILogger? log = null)
    {
        Verify.NotNull(chatfunctionConfig, "Function configuration is empty");
            ...
            SKContext context)
        {
            ...
            try
            {
                JsonArray messages = await chatfunctionConfig.PromptTemplate.RenderMessagesAsync(context);

                string chatCompletion = await client.CompleteAsync(messages, requestSettings, context.CancellationToken);
                context.Variables.Update(chatCompletion);
            }
        ...
        return new SKChatFunction(
            ...
            parameters: chatfunctionConfig.PromptTemplate.GetParameters(),
            description: chatfunctionConfig.PromptTemplateConfig.Description,
            skillName: skillName,
    ...
    /// <inheritdoc/>
    public ISKChatFunction SetDefaultSkillCollection(IReadOnlySkillCollection skills)
    {
    ...
    /// <inheritdoc/>
    public ISKChatFunction SetAIBackend(Func<IChatCompletionClient> backendFactory)
    {
    ...
    /// <inheritdoc/>
    public ISKChatFunction SetAIConfiguration(CompleteRequestSettings settings)
    {
    ...
    private IChatCompletionClient? _aiBackend = null;
    ...
    private async Task<SKContext> InvokeSemanticAsync(SKContext context, CompleteRequestSettings? settings)
    {
        ...
        var callable = (Func<IChatCompletionClient?, CompleteRequestSettings?, SKContext, Task<SKContext>>)this._function;
