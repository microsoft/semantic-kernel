...
public class OpenAIChatCompletion implements ChatCompletionService {
    ...
    public Flux<StreamingChatMessageContent> getStreamingChatMessageContentsAsync(
        ...
        List<ChatRequestMessage> messages = Collections.singletonList(
        ...
        for (String p : prompts) {
            try {
                List<ChatRequestMessage> parsedMessages = getChatRequestMessages(p);
                List<FunctionDefinition> parsedFunctions = getFunctionDefinitions(p);
