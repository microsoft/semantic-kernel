...
public class AzureCognitiveSearchMemoryStore implements MemoryStore {
  ...
  public AzureCognitiveSearchMemoryStore(@Nonnull String endpoint, @Nonnull String apiKey)
  {
    ...
    _adminClient = new SearchIndexClientBuilder()
        .endpoint(endpoint)
    ...
    return getIndexesAsync()
        .map(list -> list.stream()
            .anyMatch(name -> name.equalsIgnoreCase(collectionName) ||
                name.equalsIgnoreCase(normalizedIndexName)));
  }
  ...
  {
    if (records == null || records.isEmpty()) { return Mono.just(Collections.emptyList()); }

    collectionName = normalizeIndexName(collectionName);
    List<AzureCognitiveSearchMemoryRecord> searchRecords =
        records.stream().map(AzureCognitiveSearchMemoryRecord::fromMemoryRecord).collect(Collectors.toList());
    ...
    return client.search(null, searchOptions)
        ...
        .map(result -> {
          MemoryRecord memoryRecord =
              result
                  .getDocument(AzureCognitiveSearchMemoryRecord.class)
  ...
  public Mono<Void> removeBatchAsync(@Nonnull String collectionName, @Nonnull Collection<String> keys)
  {
    ...
    List<SearchDocument> documents = keys.stream()
  ...
  static List<SearchField> searchFields(String configName, int embeddingSize) {
    return Arrays.asList(
        ...
        new SearchField(AzureCognitiveSearchMemoryRecord.DESCRIPTION, SearchFieldDataType.STRING)
            .setSearchable(true)
            .setFacetable(true)
            .setAnalyzerName(LexicalAnalyzerName.EN_LUCENE),
        new SearchField(AzureCognitiveSearchMemoryRecord.ADDITIONAL_METADATA, SearchFieldDataType.STRING)
            .setSearchable(true)
            .setFacetable(true)
            .setAnalyzerName(LexicalAnalyzerName.EN_LUCENE),
        new SearchField(AzureCognitiveSearchMemoryRecord.EXTERNAL_SOURCE_NAME, SearchFieldDataType.STRING)
            .setSearchable(true)
            .setFacetable(true)
            .setAnalyzerName(LexicalAnalyzerName.EN_LUCENE),
        new SearchField(AzureCognitiveSearchMemoryRecord.IS_REFERENCE, SearchFieldDataType.BOOLEAN)
            .setFilterable(true)
            .setFacetable(true)
    );
  }
      ...
      int embeddingSize)
  {
    if (embeddingSize < 1) {
      throw new AzureCognitiveSearchMemoryException(
          ErrorCodes.INVALID_EMBEDDING_SIZE,
    ...
    VectorSearch vectorSearch = new VectorSearch()
        .setAlgorithmConfigurations(Collections.singletonList(
            new HnswVectorSearchAlgorithmConfiguration(SEARCH_CONFIG_NAME)
                .setParameters(new HnswParameters().setMetric(VectorSearchAlgorithmMetric.COSINE))));

    SearchIndex newIndex = new SearchIndex(normalizedIndexName)
  ...
  {
    return _adminClient.listIndexes()
  ...
  private Mono<String> upsertRecordAsync(
      @Nonnull String indexName,
      @Nonnull AzureCognitiveSearchMemoryRecord record)
  {
  ...
  private Mono<Collection<String>> upsertBatchAsync(
      @Nonnull String indexName,
      @Nonnull List<AzureCognitiveSearchMemoryRecord> records)
  {

    if (records.isEmpty()) { return Mono.just(Collections.emptyList()); }
    ...
    List<SearchDocument> documents = records.stream()
        .map(record -> {
          ...
          searchDocument.put(AzureCognitiveSearchMemoryRecord.ADDITIONAL_METADATA, record.getAdditionalMetadata());
          searchDocument.put(AzureCognitiveSearchMemoryRecord.EXTERNAL_SOURCE_NAME, record.getExternalSourceName());
          searchDocument.put(AzureCognitiveSearchMemoryRecord.IS_REFERENCE, record.isReference());
          return searchDocument;
        })
        .collect(Collectors.toList());
    ...
    return client.uploadDocumentsWithResponse(documents, options)
        .map(response -> {
          if (response.getStatusCode() == 404) {
            throw new AzureCognitiveSearchMemoryException(ErrorCodes.INVALID_INDEX_NAME);
          }
          return response;
        })
        .onErrorResume(e ->
            createIndexAsync(normalizedIndexName, embeddingSize)
                .then(client.uploadDocumentsWithResponse(documents, options))
        )
        .map(response -> response.getValue()
              .getResults()
              .stream()
              .map(IndexingResult::getKey).collect(Collectors.toList())
        );
  }
  ...
  {
    String normalizedIndexName = normalizeIndexName(indexName);
    return _clientsByIndex.computeIfAbsent(normalizedIndexName, _adminClient::getSearchAsyncClient);
  }
