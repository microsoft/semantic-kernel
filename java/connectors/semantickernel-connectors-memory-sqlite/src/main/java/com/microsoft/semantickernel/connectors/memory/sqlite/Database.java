...
public class Database {
  ...
  private static final String INDEX_NAME = "SKMemoryIndex";
    ...
    public Mono<Void> createCollectionAsync(Connection connection, String collectionName) {
          return Mono.fromRunnable(
                        () -> {
                            String query = "INSERT INTO " + COLLECTIONS_TABLE_NAME + " (name) VALUES (?)";
                        ...
                        () -> {
                            String query =
                                    "UPDATE "
                                            ...
                                            + " WHERE collection = (SELECT id FROM " + COLLECTIONS_TABLE_NAME + " WHERE name = ?)"
                                            + " AND key = ?";
                            try (PreparedStatement statement = connection.prepareStatement(query)) {
                        ...
                        () -> {
                            String query =
                                    "INSERT OR IGNORE INTO "
                                            ...
                                            + " VALUES ((SELECT id FROM " + COLLECTIONS_TABLE_NAME + " WHERE name = ?), ?, ?, ?, ?)";
                            try (PreparedStatement statement = connection.prepareStatement(query)) {
        ...
        return Mono.defer(
                        () -> {
                            ...
                            String query = "SELECT * FROM " + TABLE_NAME
                                + " WHERE collection ="
                                + " (SELECT id FROM " + COLLECTIONS_TABLE_NAME + " WHERE name = ?)";
                            try (PreparedStatement statement = connection.prepareStatement(query)) {
                                statement.setString(1, collectionName);
                                ResultSet resultSet = statement.executeQuery();
                                while (resultSet.next()) {
        ...
        return Mono.fromRunnable(
                        () -> {
                            String query1 = "DELETE FROM " + TABLE_NAME
                                + " WHERE collection ="
                                + " (SELECT id FROM " + COLLECTIONS_TABLE_NAME + " WHERE name = ?)";
                            String query2 = "DELETE FROM " + COLLECTIONS_TABLE_NAME + " WHERE name = ?";
                            try (PreparedStatement statement = connection.prepareStatement(query1);
                                ...
                                if (statement2.executeUpdate() == 0) {
                                    throw new MemoryException(ErrorCodes.ATTEMPTED_TO_ACCESS_NONEXISTENT_COLLECTION);
                                }
                            } catch (SQLException e) {
