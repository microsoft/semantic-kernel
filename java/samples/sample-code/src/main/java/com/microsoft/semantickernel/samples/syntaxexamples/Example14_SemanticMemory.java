...
import com.microsoft.semantickernel.connectors.ai.openai.util.AzureCredentialType;
...
public class Example14_SemanticMemory {
  ...
  public static void main(String[] args) throws Exception {
    ...
    AzureCognitiveSearchMemoryStore memoryStore = buildAzureCognitiveSearchMemoryStore();
  ...
  private static AzureCognitiveSearchMemoryStore buildAzureCognitiveSearchMemoryStore()
      throws ConfigurationException {
    Map<String, String> settings = SettingsMap.getDefault();

    AzureCognitiveSearchMemoryStore memoryStore;

    if (settings.get("cognativesearch.token") == null) {
      // Attempt to obtain token via
      TokenCredential token = AzureCredentialType.getCredential();
      if (token == null) {
        throw new IllegalStateException(
            "Unable to obtain token credentials. Please provide a key for Azure Cognitive Search, or log in to az.");
      }
      memoryStore = new AzureCognitiveSearchMemoryStore(
          settings.get("cognativesearch.endpoint"),
          token
      );
    } else {
      memoryStore = new AzureCognitiveSearchMemoryStore(
          settings.get("cognativesearch.endpoint"),
          settings.get("cognativesearch.token")
      );
    }
    return memoryStore;
  }
