"""Cryptographic identity for AgentMesh."""
...
class CMVKIdentity:
    ...
    def generate(cls, agent_name: str, capabilities: Optional[List[str]] = None) -> "CMVKIdentity":
        """Generate new identity with Ed25519 keys."""
    ...
    def sign(self, data: str) -> CMVKSignature:
        ...
        if CRYPTO_AVAILABLE:
            ...
            signature_b64 = base64.b64encode(hashlib.sha256(sig_input.encode()).digest()).decode("ascii")
    ...
    def public_identity(self) -> "CMVKIdentity":
        """Return copy without private key."""
        return CMVKIdentity(
            did=self.did,
            agent_name=self.agent_name,
            public_key=self.public_key,
