...
# MiniMax-M3 supports a 1,000,000 token context window.
DEFAULT_MINIMAX_CHAT_MODEL = "MiniMax-M3"


@experimental
class MiniMaxChatCompletion(MiniMaxHandler, ChatCompletionClientBase):
    """MiniMax Chat completion class.

    This class connects to the MiniMax OpenAI-compatible chat completion endpoint.
    The region can be selected through ``MiniMaxSettings`` to target the global
    (``https://api.minimax.io/v1``) or China (``https://api.minimaxi.com/v1``) endpoint.
    """
    ...
    def __init__(
        ...
        region: Literal["global_en", "cn_zh"] | None = None,
        ...
        Args:
            ai_model_id (str): MiniMax model name, for example, MiniMax-M3 or MiniMax-M2.7.
                If not provided, defaults to DEFAULT_MINIMAX_CHAT_MODEL.
            api_key (str | None): The optional API key to use. If provided will override,
                the env vars or .env file value.
            base_url (str | None): Custom API endpoint. When not provided it is resolved
                from the selected region. (Optional)
            region (Literal["global_en", "cn_zh"] | None): The MiniMax region to target.
                Defaults to "global_en". (Optional)
        ...
        if not client and not minimax_settings.api_key:
            raise ServiceInitializationError("The MiniMax API key is required.")
        if not minimax_settings.chat_model_id:
            minimax_settings.chat_model_id = DEFAULT_MINIMAX_CHAT_MODEL
            logger.warning(f"Default chat model set as: {minimax_settings.chat_model_id}")

        if not client:
            client = AsyncOpenAI(
                api_key=minimax_settings.api_key.get_secret_value() if minimax_settings.api_key else None,
                base_url=minimax_settings.base_url,
            )

        super().__init__(
            ai_model_id=minimax_settings.chat_model_id,
            api_key=minimax_settings.api_key.get_secret_value() if minimax_settings.api_key else None,
            base_url=minimax_settings.base_url,
    ...
    @classmethod
    def from_dict(cls: type["MiniMaxChatCompletion"], settings: dict[str, Any]) -> "MiniMaxChatCompletion":
        """Initialize a MiniMax service from a dictionary of settings.
    ...
    def get_prompt_execution_settings_class(self) -> type["PromptExecutionSettings"]:
        return MiniMaxChatPromptExecutionSettings

    @override
    @trace_chat_completion("minimax")
    ...
    @override
    @trace_streaming_chat_completion("minimax")
