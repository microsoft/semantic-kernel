...
def update_settings_from_function_call_configuration(
    ...
    if function_choice_configuration.required_functions:
        ...
        settings.tool_choice = function_choice_configuration.required_functions[
            0
        ].fully_qualified_name
        return
    if function_choice_configuration.functions:
        settings.tool_choice = (
            "auto" if len(function_choice_configuration.functions) > 0 else None
        )
        settings.tools = [
            kernel_function_metadata_to_function_call_format(f)
            for f in function_choice_configuration.functions
        ]
