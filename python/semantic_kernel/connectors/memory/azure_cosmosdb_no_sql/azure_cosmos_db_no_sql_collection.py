        ...
        async with self.cosmos_client:
            try:
                results = await asyncio.gather(*[
                    self.container_proxy.read_item(item=key, partition_key=get_partition_key(key)) for key in keys
                ])
        ...
        async with self.cosmos_client:
            try:
                await asyncio.gather(*[
                    self.container_proxy.delete_item(item=key, partition_key=get_partition_key(key)) for key in keys
                ])
            except Exception as e:
