...
from semantic_kernel.connectors.memory.opengauss.utils import (
    convert_dict_to_row,
    convert_row_to_dict,
    get_vector_distance_ops_str,
    get_vector_index_ops_str,
    python_type_to_opengauss,
)
...
@experimental
class OpenGaussCollection(
    VectorStoreRecordCollection[TKey, TModel],
    VectorizedSearchMixin[TKey, TModel],
    Generic[TKey, TModel],
):
    """openGauss collection implementation."""
    ...
    _settings: OpenGaussSettings = PrivateAttr()
    """openGauss settings"""
    ...
    def __init__(
        ...
        settings: OpenGaussSettings | None = None,
    ):
        ...
        self._settings = settings or OpenGaussSettings(env_file_path=env_file_path, env_file_encoding=env_file_encoding)
    ...
    @override
    async def __aenter__(self) -> "OpenGaussCollection":
    ...
    async def create_collection(self, **kwargs: Any) -> None:
        """Create a openGauss table based on a dictionary of VectorStoreRecordField.
        ...
        for field_name, field in self.data_model_definition.fields.items():
            ...
            # If the property type represents a Python type, convert it to a openGauss type
            property_type = python_type_to_opengauss(field.property_type) or field.property_type.upper()
        ...
        logger.info(f"openGauss table '{table_name}' created successfully.")
