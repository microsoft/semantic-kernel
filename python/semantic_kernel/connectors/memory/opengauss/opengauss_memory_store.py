...
from semantic_kernel.connectors.memory.opengauss.constants import DEFAULT_SCHEMA, MAX_DIMENSIONALITY
...
@experimental
class OpenGaussMemoryStore(MemoryStoreBase):
    """A memory store that uses openGauss with pgvector as the backend."""
    ...
    def __init__(
        ...
        """Initializes a new instance of the OpenGaussMemoryStore class.

        Args:
            connection_string: The connection string to the openGauss database.
        ...
        """
        try:
            opengauss_settings = OpenGaussSettings(
                connection_string=connection_string,
                env_file_path=env_file_path,
                env_file_encoding=env_file_encoding,
            )
        except ValidationError as ex:
            raise MemoryConnectorInitializationError("Failed to create opneGauss settings.", ex) from ex

        min_pool = min_pool or opengauss_settings.min_pool
        max_pool = max_pool or opengauss_settings.max_pool

        self._check_dimensionality(default_dimensionality)

        self._default_dimensionality = default_dimensionality
        self._connection_pool = ConnectionPool(
            min_size=min_pool, max_size=max_pool, open=True, kwargs=opengauss_settings.get_connection_args()
        )
        self._schema = schema
    ...
    def __enter__(self) -> "OpenGaussMemoryStore":
