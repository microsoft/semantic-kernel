...
def python_type_to_opengauss(python_type_str: str) -> str | None:
    """Convert a string representation of a Python type to a openGauss data type.

    Args:
        python_type_str: The string representation of the Python type (e.g., "int", "List[str]").

    Returns:
        Corresponding openGauss data type as a string, if found. If the type is not found, return None.
    """
    # Basic type mapping from Python types (in string form) to openGauss types
    type_mapping = {
        "str": "TEXT",
        "int": "INTEGER",
    ...
    if match:
        # Extract the inner type of the list and convert it to a openGauss array type
        element_type_str = match.group(1)
        opengauss_element_type = python_type_to_opengauss(element_type_str)
        return f"{opengauss_element_type}[]"
...
def convert_row_to_dict(
    ...
    """Convert a row from a openGauss query to a dictionary.

    Uses the field information to map the row values to the corresponding field names.

    Args:
        row: A row from a openGauss query, represented as a tuple.
...
def convert_dict_to_row(record: dict[str, Any], fields: list[tuple[str, VectorStoreRecordField]]) -> tuple[Any, ...]:
    """Convert a dictionary to a row for a openGauss query.
...
def get_vector_index_ops_str(distance_function: DistanceFunction) -> str:
    """Get the openGauss ops string for creating an index for a given distance function.

    Args:
        distance_function: The distance function the index is created for.

    Returns:
        The openGauss ops string for the given distance function.
...
def get_vector_distance_ops_str(distance_function: DistanceFunction) -> str:
    """Get the openGauss distance operator string for a given distance function.
    ...
    Note:
        ...
        since openGauss only supports ASC order index scans on operators
        For cosine similarity, take 1 - cosine distance.

    Returns:
        The openGauss distance operator string for the given distance function.
