...
from semantic_kernel.contents.const import RESPONSE_FUNCTION_RESULT_CONTENT_TAG, TEXT_CONTENT_TAG, ContentTypes
...
_T = TypeVar("_T", bound="ResponseFunctionResultContent")


class ResponseFunctionResultContent(KernelContent):
    """This class represents function result content."""

    content_type: Literal[ContentTypes.RESPONSE_FUNCTION_RESULT_CONTENT] = Field(
        RESPONSE_FUNCTION_RESULT_CONTENT_TAG, init=False
    )  # type: ignore
    tag: ClassVar[str] = RESPONSE_FUNCTION_RESULT_CONTENT_TAG
    id: str
    result: Any
    name: str | None = None
    function_name: str
    ...
    def from_function_call_content_and_result(
        ...
        result: "FunctionResult | TextContent | ResponseMessageContent | Any",
        ...
        if isinstance(result, TextContent):
            ...
            elif isinstance(result.items[0], ResponseFunctionResultContent):
                res = result.items[0].result
            res = str(result)
        else:
            res = result
        return cls(
            id=function_call_content.call_id or "unknown",
            inner_content=inner_content,
            result=res,
    ...
    def to_chat_message_content(self) -> "ResponseMessageContent":
