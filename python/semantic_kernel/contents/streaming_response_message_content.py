...
class StreamingResponseMessageContent(ResponseMessageContent, StreamingContentMixin):
    ...
    Args:
        ...
        role: Optional[AuthorRole] - The role of the message, defaults to ASSISTANT.
    ...
    def __add__(self, other: "StreamingResponseMessageContent") -> "StreamingResponseMessageContent":
        """When combining two StreamingResponseMessageContent instances, the content fields are combined.
        ...
        """
        if not isinstance(other, StreamingResponseMessageContent):
            raise ContentAdditionException(
                f"Cannot add other type to StreamingResponseMessageContent, type supplied: {type(other)}"
            )
        if self.choice_index != other.choice_index:
            raise ContentAdditionException("Cannot add StreamingResponseMessageContent with different choice_index")
        if self.ai_model_id != other.ai_model_id:
            raise ContentAdditionException("Cannot add StreamingResponseMessageContent from different ai_model_id")
        if self.encoding != other.encoding:
            raise ContentAdditionException("Cannot add StreamingResponseMessageContent with different encoding")
        if self.role and other.role and self.role != other.role:
            raise ContentAdditionException("Cannot add StreamingResponseMessageContent with different role")
    ...
    def to_element(self) -> "Element":
        """Convert the StreamingResponseMessageContent to an XML Element.

        Args:
            root_key: str - The key to use for the root of the XML Element.

        Returns:
            Element - The XML Element representing the StreamingResponseMessageContent.
        """
    ...
    def __hash__(self) -> int:
        """Return the hash of the streaming response message content."""
