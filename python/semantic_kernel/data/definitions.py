...
class VectorStoreKeyField(VectorStoreFieldBase):
...
@release_candidate
@dataclass(kw_only=True)
class VectorStoreDataField(VectorStoreFieldBase):
...
@release_candidate
@dataclass(kw_only=True)
class VectorStoreVectorField(VectorStoreFieldBase):
...
VectorStoreRecordFields = VectorStoreKeyField | VectorStoreDataField | VectorStoreVectorField
...
class VectorStoreCollectionDefinition(KernelBaseModel):
    ...
    @property
    def key_field(self) -> "VectorStoreKeyField":
        """Get the key field."""
    ...
    @property
    def vector_fields(self) -> list["VectorStoreVectorField"]:
        """Get the names of the vector fields."""
        return [field for field in self.fields if isinstance(field, VectorStoreVectorField)]

    @property
    def data_fields(self) -> list["VectorStoreDataField"]:
        """Get the names of the vector fields."""
        return [field for field in self.fields if isinstance(field, VectorStoreDataField)]

    @property
    def vector_field_names(self) -> list[str]:
        """Get the names of the vector fields."""
        return [field.name for field in self.fields if isinstance(field, VectorStoreVectorField)]

    @property
    def data_field_names(self) -> list[str]:
        """Get the names of all the data fields."""
        return [field.name for field in self.fields if isinstance(field, VectorStoreDataField)]

    def try_get_vector_field(self, field_name: str | None = None) -> VectorStoreVectorField | None:
    ...
    def get_storage_names(self, include_vector_fields: bool = True, include_key_field: bool = True) -> list[str]:
        ...
        return [
            ...
            for field in self.fields
            if (include_vector_fields or not isinstance(field, VectorStoreVectorField))
            and (include_key_field or field.name != self.key_name)
        ]
...
def _parse_vector_store_record_field_class(
    field_type: type[VectorStoreFieldBase], field: Parameter
...
def _parse_vector_store_record_field_instance(
    record_field: VectorStoreFieldBase, field: Parameter
) -> VectorStoreFieldBase:
    ...
    if not record_field.type_ and hasattr(field.annotation, "__origin__"):
        property_type = field.annotation.__origin__
        if isinstance(record_field, VectorStoreVectorField):
...
def _parse_parameter_to_field(field: Parameter) -> VectorStoreFieldBase | None:
    ...
    if field.annotation is not _empty and hasattr(field.annotation, "__metadata__"):
        for field_annotation in field.annotation.__metadata__:
            if isinstance(field_annotation, VectorStoreFieldBase):
                return _parse_vector_store_record_field_instance(field_annotation, field)
            if isinstance(field_annotation, type(VectorStoreFieldBase)):
