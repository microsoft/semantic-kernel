...
class FunctionCallingStepwisePlanner(KernelBaseModel):
    ...
    def __init__(self, service_id: str, options: Optional[FunctionCallingStepwisePlannerOptions] = None):
        ...
        super().__init__(
            service_id=service_id, options=options, generate_plan_yaml=generate_plan_yaml, step_prompt=step_prompt
        )
    ...
    async def execute(
        ...
        for i in range(self.options.max_iterations):
            ...
            chat_result = await self.get_completion_with_functions(
    ...
    async def get_completion_with_functions(
        ...
        execution_settings.auto_invoke_kernel_functions = True
        arguments = KernelArguments(kernel=kernel)
        return await chat_completion.complete_chat(
            chat_history=chat_history, settings=execution_settings, arguments=arguments
        )
    ...
    async def generate_plan(
        ...
        functions_manual = await SequentialPlannerKernelExtension.get_functions_manual(kernel, arguments)
