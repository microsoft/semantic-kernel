...
class ActionPlanner:
    ...
    def __init__(
        ...
        prompt_template = prompt if prompt is not None else ActionPlanner.ACTION_PLANNER_PROMPT
    ...
    async def create_plan(self, goal: str) -> Plan:
        ...
        elif not result_dict["plan"]["function"] or result_dict["plan"]["function"] is not None:
            new_plan = Plan(
                description=goal,
                state=variables,
                function=self._context.skills.get_function(result_dict["plan"]["function"])
            )
        else:
