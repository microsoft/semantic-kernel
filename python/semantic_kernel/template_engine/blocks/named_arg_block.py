...
class NamedArgBlock(Block):
    ...
    value: Optional[Union[VarBlock, ValBlock]] = None
    ...
    def parse_content(cls, fields: Any) -> Any:
        ...
        if not matches:
            raise ValueError("The content should be a valid named arguments block.")
        matches_dict = matches.groupdict()
        if name := matches_dict.get("name"):
            fields["name"] = name
        if value := matches_dict.get("value"):
            if matches_dict.get("var_name"):
                fields["value"] = VarBlock(content=value, name=matches_dict["var_name"])
