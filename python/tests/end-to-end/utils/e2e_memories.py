...
async def simple_memory_test(kernel: sk.Kernel):
    ...
    result = await kernel.memory.search_async("test", "What are mammals?", limit=2, min_relevance_score=0.0)
    ...
    result = await kernel.memory.search_async("test", "What are fish?", limit=1, min_relevance_score=0.0)
    print(f"#1 Answer: {result[0].text}\n")
    assert result[0].text == "Sharks are fish."

    result = await kernel.memory.search_async("test", "What are insects?", limit=1, min_relevance_score=0.0)
    print(f"#1 Answer: {result[0].text}\n")
    assert result[0].text == "Flies are insects."

    result = await kernel.memory.search_async("test", "What are birds?", limit=1, min_relevance_score=0.0)
    print(f"#1 Answer: {result[0].text}\n")
