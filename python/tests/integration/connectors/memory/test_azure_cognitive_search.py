...
@pytest.fixture
def memory_store():
    yield AzureCognitiveSearchMemoryStore(vector_size=4)


def test_constructor():
    test_endpoint = "https://test-endpoint.search.windows.net"
    memory = AzureCognitiveSearchMemoryStore(test_endpoint)
...
@pytest.mark.asyncio
async def test_record_not_found(memory_store):
    n = randint(1000, 9999)
    collection = f"int-tests-{n}"
    await memory_store.create_collection_async(collection)
    time.sleep(1)
    ...
    try:
        ...
        # KeyError eException should occur
