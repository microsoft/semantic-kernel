...
from semantic_kernel.connectors.memory.opengauss import OpenGaussMemoryStore
...
def connection_string():
    try:
        ...
        if opengauss_settings.connection_string is not None:
            return opengauss_settings.connection_string.get_secret_value()
    except ValidationError:
        pytest.skip("OpenGauss Connection string not found in env vars.")
    pytest.skip("OpenGauss Connection string not found in env vars.")


def test_constructor(connection_string):
    with OpenGaussMemoryStore(connection_string, 2, 1, 5) as memory:
