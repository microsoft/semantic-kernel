...
async def test_should_agent_terminate_with_none_result():
    ...
    strategy = KernelFunctionTerminationStrategy(
        function=mock_function, kernel=mock_kernel, result_parser=lambda result: result.value if result else False
    )
...
async def test_should_agent_terminate_custom_arguments():
    ...
    strategy = KernelFunctionTerminationStrategy(
        function=mock_function, kernel=mock_kernel, arguments=custom_args, result_parser=lambda result: result.value
    )

    with patch.object(KernelArguments, "__init__", return_value=None) as mock_init:
        result = await strategy.should_agent_terminate(agent, history)
        mock_init.assert_called_once_with(**{
            "some_setting": {"key": "value"},
            "_agent_": agent.name or agent.id,
            "_history_": [message.to_dict(role_key="role", content_key="content") for message in history],
        })
...
async def test_should_agent_terminate_result_parser_awaitable():
    ...
    strategy = KernelFunctionTerminationStrategy(
        function=mock_function, kernel=mock_kernel, result_parser=mock_result_parser
    )
