...
def test_google_ai_text_completion_init(google_ai_unit_test_env) -> None:
    ...
    assert (
        google_ai_text_completion.service_settings.api_key.get_secret_value() == api_key
    )


def test_google_ai_text_completion_init_with_service_id(
...
def test_google_ai_text_completion_init_with_model_id_in_argument(
...
def test_google_ai_text_completion_init_with_empty_model_id(
    google_ai_unit_test_env,
) -> None:
...
def test_google_ai_text_completion_init_with_empty_api_key(
    google_ai_unit_test_env,
) -> None:
...
def test_prompt_execution_settings_class(google_ai_unit_test_env) -> None:
    google_ai_text_completion = GoogleAITextCompletion()
    assert (
        google_ai_text_completion.get_prompt_execution_settings_class()
        == GoogleAITextPromptExecutionSettings
    )
...
async def test_google_ai_text_completion(
    ...
    mock_google_model_generate_content_async.return_value = (
        mock_google_ai_text_completion_response
    )
    ...
    assert (
        responses[0].text
        == mock_google_ai_text_completion_response.candidates[0].content.parts[0].text
    )
