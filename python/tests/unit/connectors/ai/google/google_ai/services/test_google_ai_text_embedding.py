...
def test_google_ai_text_embedding_init(google_ai_unit_test_env) -> None:
    ...
    assert (
        google_ai_text_embedding.service_settings.api_key.get_secret_value() == api_key
    )


def test_google_ai_text_embedding_init_with_service_id(
...
def test_google_ai_text_embedding_init_with_model_id_in_argument(
    ...
    google_ai_chat_completion = GoogleAITextEmbedding(
...
@pytest.mark.parametrize(
    "exclude_list", [["GOOGLE_AI_EMBEDDING_MODEL_ID"]], indirect=True
)
def test_google_ai_text_embedding_init_with_empty_model_id(
    google_ai_unit_test_env,
) -> None:
...
def test_google_ai_text_embedding_init_with_empty_api_key(
    google_ai_unit_test_env,
) -> None:
...
def test_prompt_execution_settings_class(google_ai_unit_test_env) -> None:
    google_ai_text_embedding = GoogleAITextEmbedding()
    assert (
        google_ai_text_embedding.get_prompt_execution_settings_class()
        == GoogleAIEmbeddingPromptExecutionSettings
    )
...
async def test_embedding_with_settings(
    mock_embedding_client, google_ai_unit_test_env, prompt
):
    """Test that the service initializes and generates embeddings correctly."""
    model_id = google_ai_unit_test_env["GOOGLE_AI_EMBEDDING_MODEL_ID"]
...
async def test_embedding_list_input(
    ...
    mock_embedding_client.assert_called_once_with(
        model=model_id, content=[prompt, prompt]
    )
...
async def test_raw_embedding(mock_embedding_client, google_ai_unit_test_env, prompt):
    ...
    response: list[list[float]] = (
        await google_ai_text_embedding.generate_raw_embeddings(
            [prompt],
            settings=settings,
        )
    )
