...
def test_finish_reason_from_vertex_ai_to_semantic_kernel():
    ...
    assert (
        finish_reason_from_vertex_ai_to_semantic_kernel(Candidate.FinishReason.STOP)
        == FinishReason.STOP
    )
    assert (
        finish_reason_from_vertex_ai_to_semantic_kernel(
    ...
    assert (
        finish_reason_from_vertex_ai_to_semantic_kernel(Candidate.FinishReason.SAFETY)
        == FinishReason.CONTENT_FILTER
    )
    assert (
        finish_reason_from_vertex_ai_to_semantic_kernel(Candidate.FinishReason.OTHER)
        is None
    )
