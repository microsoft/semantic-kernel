...
def test_vertex_ai_text_completion_init_with_service_id(
...
def test_vertex_ai_text_completion_init_with_model_id_in_argument(
...
def test_vertex_ai_text_completion_init_with_empty_model_id(
    vertex_ai_unit_test_env,
) -> None:
...
def test_vertex_ai_text_completion_init_with_empty_project_id(
    vertex_ai_unit_test_env,
) -> None:
...
def test_prompt_execution_settings_class(vertex_ai_unit_test_env) -> None:
    vertex_ai_text_completion = VertexAITextCompletion()
    assert (
        vertex_ai_text_completion.get_prompt_execution_settings_class()
        == VertexAITextPromptExecutionSettings
    )
...
async def test_vertex_ai_text_completion(
    ...
    vertex_ai_text_completion = VertexAITextCompletion()
    responses: list[TextContent] = await vertex_ai_text_completion.get_text_contents(
        prompt, settings
    )
    ...
    assert (
        responses[0].text
        == mock_vertex_ai_text_completion_response.candidates[0].content.parts[0].text
    )
...
async def test_vertex_ai_streaming_text_completion(
    ...
    vertex_ai_text_completion = VertexAITextCompletion()
    async for chunks in vertex_ai_text_completion.get_streaming_text_contents(
        prompt, settings
    ):
        assert len(chunks) == 1
