...
def mock_mistral_ai_client_completion() -> MistralAsyncClient:
    ...
    choices = [
        MagicMock(
            finish_reason="stop", message=MagicMock(role="assistant", content="Test")
        )
    ]
...
def mock_mistral_ai_client_completion_stream() -> MistralAsyncClient:
    ...
    choices = [
        MagicMock(
            finish_reason="stop", delta=MagicMock(role="assistant", content="Test")
        ),
        MagicMock(
            finish_reason="stop",
            delta=MagicMock(role="assistant", content="Test", tool_calls=None),
        ),
    ]
    chat_completion_response.choices = choices
    ...
    generator_mock.__aiter__.return_value = [
        chat_completion_response_empty,
...
@pytest.mark.asyncio
async def test_mistral_ai_sdk_exception(
    kernel: Kernel, mock_settings: MistralAIChatPromptExecutionSettings
):
    chat_history = MagicMock()
    arguments = KernelArguments()
    client = MagicMock(spec=MistralAsyncClient)
...
@pytest.mark.asyncio
async def test_mistral_ai_sdk_exception_streaming(
    kernel: Kernel, mock_settings: MistralAIChatPromptExecutionSettings
):
    chat_history = MagicMock()
    arguments = KernelArguments()
...
def test_mistral_ai_chat_completion_init(mistralai_unit_test_env) -> None:
    ...
    assert (
        mistral_ai_chat_completion.ai_model_id
        == mistralai_unit_test_env["MISTRALAI_CHAT_MODEL_ID"]
    )
    assert (
        mistral_ai_chat_completion.async_client._api_key
        == mistralai_unit_test_env["MISTRALAI_API_KEY"]
    )
    assert isinstance(mistral_ai_chat_completion, ChatCompletionClientBase)


@pytest.mark.parametrize(
    "exclude_list", [["MISTRALAI_API_KEY", "MISTRALAI_CHAT_MODEL_ID"]], indirect=True
)
...
def test_mistral_ai_chat_completion_init_constructor_missing_model(
...
def test_mistral_ai_chat_completion_init_constructor_missing_api_key(
    mistralai_unit_test_env,
) -> None:
    # Test successful initialization
    with pytest.raises(ServiceInitializationError):
...
def test_mistral_ai_chat_completion_init_with_empty_model_id(
    mistralai_unit_test_env,
) -> None:
    with pytest.raises(ServiceInitializationError):
        MistralAIChatCompletion(
...
def test_prompt_execution_settings_class(mistralai_unit_test_env):
    ...
    prompt_execution_settings = (
        mistral_ai_chat_completion.get_prompt_execution_settings_class()
    )
...
async def test_with_different_execution_settings(
    kernel: Kernel, mock_mistral_ai_client_completion: MagicMock
):
    chat_history = MagicMock()
...
async def test_with_different_execution_settings_stream(
    ...
    async for chunk in chat_completion_base.get_streaming_chat_message_contents(
        ...
        continue
    assert (
        mock_mistral_ai_client_completion_stream.chat_stream.call_args.kwargs[
            "temperature"
        ]
        == 0.2
    )
    assert (
        mock_mistral_ai_client_completion_stream.chat_stream.call_args.kwargs["seed"]
        == 2
    )
