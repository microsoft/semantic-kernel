...
def test_init_minimal_prompt_template():
    function = KernelFunctionFromPrompt(
        ...
        prompt_template=KernelPromptTemplate(
            prompt_template_config=PromptTemplateConfig(template="test")
        ),
    )
...
def test_init_invalid_name():
    with pytest.raises(FunctionInitializationError):
        KernelFunctionFromPrompt(
            function_name="test func", plugin_name="test", prompt="test"
        )
    ...
    function = KernelFunctionFromPrompt(
        ...
        prompt_template=KernelPromptTemplate(
            prompt_template_config=PromptTemplateConfig(
                template="test", execution_settings={}
            )
        ),
        prompt_execution_settings=None,
    )
...
async def test_invoke_chat_stream(openai_unit_test_env):
    ...
    with patch(
        ...
        mock.return_value = [
            ChatMessageContent(role=AuthorRole.ASSISTANT, content="test", metadata={})
        ]
    ...
    with patch(
        ...
        mock.return_value = [
            StreamingChatMessageContent(
                choice_index=0, role=AuthorRole.ASSISTANT, content="test", metadata={}
            )
        ]
...
async def test_invoke_defaults(openai_unit_test_env):
    ...
    function = KernelFunctionFromPrompt(
        ...
        prompt_template_config=PromptTemplateConfig(
            ...
            input_variables=[
                InputVariable(
                    name="input", type="str", default="test", is_required=False
                )
            ],
        ),
        prompt_execution_settings=PromptExecutionSettings(service_id="test"),
...
def test_create_with_multiple_settings():
    ...
    assert (
        function.prompt_template.prompt_template_config.execution_settings[
            "test"
        ].extension_data["temperature"]
        == 0.0
    )
    assert (
        function.prompt_template.prompt_template_config.execution_settings[
...
async def test_prompt_render(kernel: Kernel, openai_unit_test_env):
    ...
    context = FunctionInvocationContext(
        function=function, kernel=kernel, arguments=KernelArguments()
    )
