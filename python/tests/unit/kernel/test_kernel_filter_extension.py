...
class TestKernelFilterExtension:
    def test_add_filter(
        self, kernel: Kernel, custom_filter, filter_type: str, filter_attr: str
    ):
        kernel.add_filter(filter_type, custom_filter)
        assert len(getattr(kernel, filter_attr)) == 1

    def test_add_multiple_filters(
        self, kernel: Kernel, custom_filter, filter_type: str, filter_attr: str
    ):
    ...
    def test_remove_filter_with_type(
        self, kernel: Kernel, custom_filter, filter_type: str, filter_attr: str
    ):
        kernel.add_filter(filter_type, custom_filter)
        assert len(getattr(kernel, filter_attr)) == 1
    ...
    def test_remove_filter_by_position(
        self, kernel: Kernel, custom_filter, filter_type: str, filter_attr: str
    ):
        kernel.add_filter(filter_type, custom_filter)
        assert len(getattr(kernel, filter_attr)) == 1
    ...
    def test_remove_filter_without_type(
        self, kernel: Kernel, custom_filter, filter_type: str, filter_attr: str
    ):
        kernel.add_filter(filter_type, custom_filter)
        assert len(getattr(kernel, filter_attr)) == 1
