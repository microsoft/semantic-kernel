...
def azure_cognitive_search_memory_store(azure_ai_search_unit_test_env):
    ...
    return AzureCognitiveSearchMemoryStore(
        ...
        azure_credentials=AzureKeyCredential("test_key"),
    )
...
def mock_search_index_client():
    ...
    with patch(
        "azure.search.documents.indexes.aio.SearchIndexClient.create_index"
    ) as mock_create_index:
        # Setup the mock to return a specific SearchIndex instance when called
        mock_create_index.return_value = SearchIndex(
            name="testIndexWithEncryption", fields=[]
        )
...
def mock_encryption_key():
    ...
    return SearchResourceEncryptionKey(
        ...
        vault_uri="https://mockvault.vault.azure.net/",
    )
...
def mock_get_index_client():
    ...
    with patch(
        "azure.search.documents.indexes.aio.SearchIndexClient.get_index",
...
async def test_create_collection_with_encryption_key(
    ...
    mock_search_index_client.return_value = SearchIndex(
        name="testIndexWithEncryption",
        fields=[],
        search_resource_encryption_key=mock_encryption_key,
    )
    ...
    assert (
        created_index.encryption_key == mock_encryption_key
    ), "Encryption key was not set correctly"
