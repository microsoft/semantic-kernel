...
# @pytest.mark.asyncio
# async def test_send_outgoing_public_events_success(actor: ProcessActor):
#     """Test that send_outgoing_public_events enqueues messages based on output_edges."""
#     # Arrange: set up parent_process_id and process state
#     actor.parent_process_id = "parent1"
#     actor.id = ActorId("actor1")
#     # Provide a process with name and id for scoping
#     state = KernelProcessState(name="ProcName", version="1.0", id="P1")
#     actor.process = MagicMock(state=state)
#     # Simulate output edges for an event 'e1'
#     edge = MagicMock()
#     edge.source_step_id = "src1"
#     edge.output_target = MagicMock(
#         step_id="step1",
#         parameter_name=None,
#         function_name=None,
#         target_event_id=None,
#     )
#     actor.output_edges = {"e1": [edge, edge]}
#     # Create a ProcessEvent JSON
#     raw_event = ProcessEvent(inner_event=KernelProcessEvent(id="e1", data="data1"))
#     event_json = raw_event.model_dump_json()
#     # Mock ActorProxy.create to return event and message queues
#     event_queue = AsyncMock()
#     event_queue.dequeue_all = AsyncMock(return_value=[event_json])
#     message_queue = AsyncMock()

#     def proxy_side_effect(actor_type, actor_id, actor_interface):
#         if actor_interface is EventBufferInterface:
#             return event_queue
#         if actor_interface is MessageBufferInterface:
#             return message_queue
#         return MagicMock()

#     with patch(
#         "semantic_kernel.processes.dapr_runtime.actors.process_actor.ActorProxy.create", side_effect=proxy_side_effect
#     ):
#         # Act
#         await actor.send_outgoing_public_events()
#         # Assert: dequeue_all called, and enqueue called for each edge
#         event_queue.dequeue_all.assert_called_once()
#         assert message_queue.enqueue.call_count == 2
#         # Verify that enqueued messages are JSON strings
#         for args, _ in message_queue.enqueue.call_args_list:
#             # Each call's first argument is JSON
#             assert isinstance(args[0], str)
#             # The string should decode to a dict with keys matching ProcessMessageFactory
#             msg_dict = json.loads(args[0])
#             assert set(msg_dict.keys()) >= {"source_id", "destination_id", "function_name", "values"}
