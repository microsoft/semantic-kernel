...
st.write("You can ask questions about the data, and the AI will answer them. Upload a CSV file to get started.")
...
if uploaded_file is not None:
    ...
    df_analysis = analyzeDataframe(variables=ContextVariables(content=df_head_short)).result
    df_analyzed = True
...
if submit and uploaded_file is not None:
    ...
    prompt = decipherPrompt(variables=ContextVariables(content=prompt, variables={"dataframe": df_head_short})).result
    print("New Prompt: ", prompt)
    #Generate Code using createQuery semantic function
    response = createQuery(variables=ContextVariables(content=prompt, variables = {"dataframe": df_head_short})).result
    ...
    for i in range(3):
        try:
            ...
            code = repairCode(variables=ContextVariables(content=error_message, variables={"task":prompt, "df": df_head_short, "code": code, })).result
            print("New Code: ", code)
