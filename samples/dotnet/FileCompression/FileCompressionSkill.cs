...
public class FileCompressionSkill
{
    ...
    public async Task<string?> CompressFileAsync(string sourceFilePath, SKContext context)
    {
        ...
        {
            const string ERROR_MESSAGE = $"Missing context variable {Parameters.DestinationFilePath} in {nameof(this.CompressFileAsync)}";
            this._logger.LogError(ERROR_MESSAGE);
    ...
    public async Task<string?> CompressDirectoryAsync(string sourceDirectoryPath, SKContext context)
    {
        ...
        if (!context.Variables.Get(Parameters.DestinationFilePath, out string destinationFilePath))
        {
            const string ERROR_MESSAGE = $"Missing context variable {Parameters.DestinationFilePath} in {nameof(this.CompressDirectoryAsync)}";
            this._logger.LogError(ERROR_MESSAGE);
            context.Fail(ERROR_MESSAGE);
    ...
    public async Task<string?> DecompressFileAsync(string sourceFilePath, SKContext context)
    {
        ...
        {
            const string ERROR_MESSAGE = $"Missing context variable {Parameters.DestinationDirectoryPath} in {nameof(this.DecompressFileAsync)}";
            this._logger.LogError(ERROR_MESSAGE);
            context.Fail(ERROR_MESSAGE);
