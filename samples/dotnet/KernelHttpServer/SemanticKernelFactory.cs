        ...
        return builder.Configure(c =>
        {
            switch (config.CompletionConfig.AIService)
            {
                case AIService.OpenAI:
                    c.AddOpenAITextCompletion(config.CompletionConfig.ServiceId, config.CompletionConfig.DeploymentOrModelId, config.CompletionConfig.Key);
                    break;
                case AIService.AzureOpenAI:
                    c.AddAzureOpenAITextCompletion(config.CompletionConfig.ServiceId, config.CompletionConfig.DeploymentOrModelId, config.CompletionConfig.Endpoint,
            ...
            if (memoryStore != null && config.EmbeddingConfig.IsValid())
            {
                ...
                {
                    case AIService.OpenAI:
                        c.AddOpenAIEmbeddingGeneration(config.EmbeddingConfig.ServiceId, config.EmbeddingConfig.DeploymentOrModelId, config.EmbeddingConfig.Key);
                        break;
                    case AIService.AzureOpenAI:
                        c.AddAzureOpenAIEmbeddingGeneration(config.EmbeddingConfig.ServiceId, config.EmbeddingConfig.DeploymentOrModelId,
